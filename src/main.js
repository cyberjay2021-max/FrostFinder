...
import {
  initUndoDeps, pushUndo, undoLastOp, redoLastOp,
  undoToIndex, toggleUndoPanel, renderUndoPanel,
  persistUndoHistory, restoreUndoHistory, clearUndoHistory,
} from './undo.js';
import {
  initSettingsDeps, loadPersistentSettings, persistSettings, patchLocalStorage,
  isSettingsLoaded, setSettingsLoaded,
} from './settings.js';
import {
  initNetworkDeps, showSftpDialog, showFtpDialog, showVaultDialog, showCloudDialog,
} from './network.js';
import {
  initPluginDeps, loadPlugins, matchesGlob, pluginsForEntry, runPlugin,
  showPluginManager, _pluginDetectCapabilities, _revokePluginTrust,
...
// ── Desktop notifications (r30 P3.3) ─────────────────────────────────────────
// Fires a system notification when a file operation completes while the app
// window is not focused. Uses tauri-plugin-notification.
let _appFocused = true;
document.addEventListener('visibilitychange', () => { _appFocused = !document.hidden; });
window.addEventListener('focus', () => { _appFocused = true; });
window.addEventListener('blur',  () => { _appFocused = false; });

async function notifyOpComplete(title, body, openPath = null) {
  if (_appFocused) return; // app is visible — in-app toast is sufficient
  try {
  ...
  } catch (_) {} // non-fatal — notification API may be unavailable
}
...
function closeTab(id){
  ...
  if(tabs.length===1){
    ...
    _tabIdCounter++;
    const newTabState=makeTabState('');
    tabs[0]={id:newId,label:'New Tab',state:newTabState};
    activeTabId=newId;
    Object.assign(state,newTabState);
    renderTabs();syncState();saveSession();
    invoke('get_home_dir').then(h=>navigate(h,0,true)).catch(()=>{});
    return;
  }
  tabs.splice(idx,1);
...
function switchTab(id,doRender=true){
  syncState(); // save current tab's state before switching
  activeTabId=id;
...
// Reads ff_theme ('dark'|'light'|'system', default 'dark') and applies
// data-theme="light" to <html> when light mode is active.
function applyTheme(theme){
  theme = theme || localStorage.getItem('ff_theme') || 'dark';
...
// Wire system-preference changes when theme is 'system'
window.matchMedia('(prefers-color-scheme: light)').addEventListener('change', () => {
  if((localStorage.getItem('ff_theme') || 'dark') === 'system') applyTheme('system');
});
...
async function loadTagsForEntries(entries){
  try{
    ...
    for(const t of tagsWithColors){
      try{
        const results=await invoke('search_by_tag_v2',{tag:t.name});
...
// ── Clipboard ─────────────────────────────────────────────────────────────────
function clipboardCopy(entries){state.clipboard={entries:[...entries],op:'copy'};showToast(t('toast.copied',{n:entries.length}),'info');_addToClipboardHistory(entries,'copy');}
function clipboardCut(entries){state.clipboard={entries:[...entries],op:'cut'};showToast(t('toast.cut',{n:entries.length}),'info');_addToClipboardHistory(entries,'cut');}

// ── Clipboard history ───────────────────────────────────────────────────────────
const CLIPBOARD_HISTORY_KEY = 'ff_clipboard_history';
const CLIPBOARD_HISTORY_MAX = 20;

function _getClipboardHistory(){
  try{ return JSON.parse(localStorage.getItem(CLIPBOARD_HISTORY_KEY)||'[]'); }catch{ return []; }
}

function _saveClipboardHistory(history){
  localStorage.setItem(CLIPBOARD_HISTORY_KEY, JSON.stringify(history));
}

function _addToClipboardHistory(entries, op){
  if(!entries?.length) return;
  const history = _getClipboardHistory();
  const newItem = {
    paths: entries.map(e=>e.path),
    names: entries.map(e=>e.name),
    op,
    time: Date.now(),
    isDir: entries.some(e=>e.is_dir)
  };
  // Don't add duplicates (same paths as last entry)
  const lastItem = history[0];
  if(lastItem && JSON.stringify(lastItem.paths) === JSON.stringify(newItem.paths)){
    return;
  }
  history.unshift(newItem);
  if(history.length > CLIPBOARD_HISTORY_MAX) history.pop();
  _saveClipboardHistory(history);
}

function _showClipboardHistoryPanel(){
  const existing = document.getElementById('ff-clipboard-history');
  if(existing){ existing.remove(); return; }
  
  const history = _getClipboardHistory();
  if(!history.length){ showToast(t('toast.no_clipboard_history'),'info'); return; }
  
  const panel = document.createElement('div');
  panel.id = 'ff-clipboard-history';
  panel.style.cssText = 'position:fixed;top:60px;left:50%;transform:translateX(-50%);width:min(500px,90vw);max-height:60vh;overflow-y:auto;background:#1c1e24;border:1px solid rgba(255,255,255,.14);border-radius:12px;box-shadow:0 16px 48px rgba(0,0,0,.6);z-index:9500;';
  
  const title = document.createElement('div');
  title.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:12px 16px;border-bottom:1px solid rgba(255,255,255,.07);';
  title.innerHTML = `<span style="font-size:14px;font-weight:600;color:#f1f5f9;">Clipboard History</span>
    <button id="ff-ch-clear" style="background:none;border:none;color:#636368;cursor:pointer;font-size:12px;">Clear all</button>`;
  ...
  panel.querySelector('#ff-ch-clear').addEventListener('click', e => {
    e.stopPropagation();
  ...
  setTimeout(()=>{
    const outsideClick = ev => { if(!panel.contains(ev.target)){ panel.remove(); document.removeEventListener('click',outsideClick); }};
...
async function _pasteFromHistory(idx){
  const history = _getClipboardHistory();
  const item = history[idx];
  if(!item) return;
  document.getElementById('ff-clipboard-history')?.remove();
  
  // Set clipboard state
  state.clipboard = { entries: item.paths.map(p=>({path:p,name:p.split('/').pop()})), op: item.op };
...
function showContextMenu(x,y,items){
  ...
  // FIX (RC2-R1 Bug 5): clamp all four edges so menus never escape the viewport.
  // Previous code only clamped right and bottom; menus near the top or left edge
  // (e.g. right-clicking a file in the first row) would fly off-screen.
  const clampedLeft=Math.max(4,r.right>window.innerWidth?x-r.width:x);
  const clampedTop=Math.max(4,r.bottom>window.innerHeight?y-r.height:y);
...
function _showAdvancedSearch() {
  ...
  dlg.innerHTML = `<div style="background:#1e1e21;border:1px solid rgba(255,255,255,.13);border-radius:14px;padding:24px 26px 20px;min-width:420px;max-width:520px;box-shadow:0 16px 48px rgba(0,0,0,.75);">
    ...
    <div style="display:flex;gap:16px;margin-bottom:12px;flex-wrap:wrap;">
      <div style="flex:1;min-width:140px;">
        <label style="display:block;font-size:10px;color:#636368;margin-bottom:4px;text-transform:uppercase;">Size</label>
        <select id="adv-size-op" style="width:100%;padding:6px 8px;background:#2a2a2d;border:1px solid rgba(255,255,255,.12);border-radius:6px;color:#c9d1d9;font-size:12px;">
          <option value="">Any size</option>
          <option value="lt1kb">Less than 1 KB</option>
          <option value="lt10kb">Less than 10 KB</option>
          <option value="lt100kb">Less than 100 KB</option>
          <option value="lt1mb">Less than 1 MB</option>
          <option value="lt10mb">Less than 10 MB</option>
          <option value="lt100mb">Less than 100 MB</option>
          <option value="gt1kb">Greater than 1 KB</option>
          <option value="gt10kb">Greater than 10 KB</option>
          <option value="gt100kb">Greater than 100 KB</option>
          <option value="gt1mb">Greater than 1 MB</option>
          <option value="gt10mb">Greater than 10 MB</option>
          <option value="gt100mb">Greater than 100 MB</option>
          <option value="gt1gb">Greater than 1 GB</option>
        </select>
      </div>
      <div style="flex:1;min-width:140px;">
        <label style="display:block;font-size:10px;color:#636368;margin-bottom:4px;text-transform:uppercase;">Date modified</label>
        <select id="adv-date-op" style="width:100%;padding:6px 8px;background:#2a2a2d;border:1px solid rgba(255,255,255,.12);border-radius:6px;color:#c9d1d9;font-size:12px;">
          ...
          <option value="older1y">Older than 1 year</option>
          <option value="older6m">Older than 6 months</option>
  ...
  dlg.querySelector('#adv-save').addEventListener('click', async () => {
    ...
    arr.push({
      ...
      sizeOp: dlg.querySelector('#adv-size-op').value || null,
      dateOp: dlg.querySelector('#adv-date-op').value || null,
    });
  ...
  const doSearch = async () => {
    ...
    const sizeOp = dlg.querySelector('#adv-size-op').value;
    const dateOp = dlg.querySelector('#adv-date-op').value;
    close();
    state.searchMode=true; state.searchQuery=(useRegex?'regex: ':'')+query;
    state.searchResults=[]; state.selIdx=-1; sel.clear();
    state.loading=true; render();
    try{
      const results = await invoke('search_advanced',{query, rootPath, recursive:true, useRegex, searchContents, includeHidden, sizeOp, dateOp});
...
function toggleTerminal(){
  ...
  const existing=document.getElementById('ff-terminal-panel');
  if(_terminalOpen){
    if(existing)existing.style.display='flex';
    else _createTerminalPanel();
  }else{
    if(existing)existing.style.display='none';
  }
}
function _createTerminalPanel(){
  const p=document.createElement('div');
  p.id='ff-terminal-panel';
  p.style.cssText='position:fixed;bottom:0;left:0;right:0;height:200px;background:#0d1117;border-top:1px solid #30363d;display:flex;flex-direction:column;z-index:9000;';
  p.innerHTML=`<div style="display:flex;align-items:center;padding:6px 12px;background:#161b22;border-bottom:1px solid #30363d;gap:8px;">
    <span style="color:#58a6ff;font-size:12px;font-weight:600;">Terminal</span>
    <span style="color:#8b949e;font-size:11px;">${escHtml(state.currentPath)}</span>
    <button id="term-close" style="margin-left:auto;background:none;border:none;color:#8b949e;cursor:pointer;font-size:16px;">×</button>
  </div>
  <div id="term-output" style="flex:1;overflow:auto;padding:8px;font-family:monospace;font-size:12px;color:#c9d1d9;white-space:pre-wrap;"></div>
  <div style="display:flex;padding:8px;border-top:1px solid #30363d;">
    <span style="color:#58a6ff;margin-right:8px;">$</span>
    <input id="term-input" style="flex:1;background:none;border:none;color:#c9d1d9;font-family:monospace;font-size:12px;outline:none;" placeholder="Enter command..." autofocus>
  </div>`;
  document.body.appendChild(p);
  const out=p.querySelector('#term-output');
  const inp=p.querySelector('#term-input');
  p.querySelector('#term-close').addEventListener('click',()=>{_terminalOpen=false;p.style.display='none';});
  inp.addEventListener('keydown',async e=>{
    if(e.key==='Enter'){
      ...
      inp.value='';
      try{
        const{stdout,stderr}=await invoke('run_command',{cmd,cwd:state.currentPath});
        if(stdout)out.textContent+=stdout+'\n';
        if(stderr)out.textContent+='\x1b[31m'+stderr+'\x1b[0m\n';
      }catch(err){out.textContent+='\x1b[31mError: '+err+'\x1b[0m\n';}
 ...
 // ── Organize Favorites ───────────────────────────────────────────────────────
function _showOrganizeFavorites(){
  document.getElementById('ff-organize-favs')?.remove();
  const custom=getSidebarFavs();
  const ov=document.createElement('div');
  ov.id='ff-organize-favs';
  ...
  ov.innerHTML=`<div style="background:#1e1e21;border:1px solid rgba(255,255,255,.12);border-radius:14px;padding:24px;min-width:380px;max-width:500px;max-height:80vh;overflow-y:auto;">
    <div style="font-size:16px;font-weight:600;margin-bottom:16px;color:#f1f5f9">Organize Favorites</div>
    <div id="fav-list" style="display:flex;flex-direction:column;gap:8px;"></div>
    <div style="margin-top:20px;display:flex;gap:8px;justify-content:flex-end;">
      <button id="fav-org-close" style="padding:8px 16px;background:rgba(255,255,255,.07);border:none;border-radius:7px;color:#98989f;cursor:pointer;">Close</button>
    </div>
  </div>`;
  const list=ov.querySelector('#fav-list');
  const renderFavs=()=>{
    const favs=getSidebarFavs();
    list.innerHTML=favs.length?favs.map((f,i)=>`<div style="display:flex;align-items:center;gap:8px;padding:8px;background:rgba(255,255,255,.05);border-radius:6px;">
      <span style="cursor:grab;opacity:.5;">⠿</span>
      <span style="flex:1;overflow:hidden;text-overflow:ellipsis;">${escHtml(f.name)}</span>
      <span style="font-size:11px;color:#636368;max-width:150px;overflow:hidden;text-overflow:ellipsis;">${escHtml(f.path)}</span>
      <button class="fav-del" data-idx="${i}" style="background:none;border:none;color:#f87171;cursor:pointer;padding:2px 6px;">×</button>
    </div>`).join(''):'<div style="color:#636368;text-align:center;padding:20px;">No favorites yet</div>';
  };
  renderFavs();
  list.querySelectorAll('.fav-del').forEach(b=>{b.addEventListener('click',()=>{removeSidebarFav(getSidebarFavs()[+b.dataset.idx].path);renderFavs();renderSidebar();});});
  ov.querySelector('#fav-org-close').addEventListener('click',()=>ov.remove());
...
function _showSettings() {
  ...
  const renderContent = () => {
    ...
    switch(activeSection) {
      case 'general': c.innerHTML = `
        ...
        <div class="stg-group">
          ...
          <label class="stg-row"><span>Default icon size</span>
            <div class="ff-csel" style="min-width:120px">
              <button class="ff-csel-btn" type="button"><span class="ff-csel-label">${{Small:'Small',Medium:'Medium',Large:'Large'}[{60:'Small',80:'Medium',112:'Large'}[get('ff_icon_size','80')]]||'Medium'}</span><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
              <div class="ff-csel-popup">
                <div class="ff-csel-opt${get('ff_icon_size','80')==='60'?' active':''}" data-val="60">Small</div>
                <div class="ff-csel-opt${get('ff_icon_size','80')==='80'?' active':''}" data-val="80">Medium</div>
                <div class="ff-csel-opt${get('ff_icon_size','80')==='112'?' active':''}" data-val="112">Large</div>
              </div>
              <input type="hidden" class="stg-select" data-key="ff_icon_size" value="${get('ff_icon_size','80')}">
            </div></label>
        </div>`; break;
      case 'appearance': c.innerHTML = `
        <div class="stg-group">
          <div class="stg-label">${t('settings.theme')}</div>
          <label class="stg-row"><span>${t('settings.theme')}</span>
            <div class="ff-csel" style="min-width:140px">
              <button class="ff-csel-btn" type="button"><span class="ff-csel-label">${{dark:t('settings.theme.dark'),light:t('settings.theme.light'),system:t('settings.theme.system')}[get('ff_theme','dark')]||t('settings.theme.dark')}</span><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
              <div class="ff-csel-popup">
                <div class="ff-csel-opt${get('ff_theme','dark')==='dark'?' active':''}" data-val="dark">${t('settings.theme.dark')}</div>
                <div class="ff-csel-opt${get('ff_theme','dark')==='light'?' active':''}" data-val="light">${t('settings.theme.light')}</div>
                <div class="ff-csel-opt${get('ff_theme','dark')==='system'?' active':''}" data-val="system">${t('settings.theme.system')}</div>
              </div>
              <input type="hidden" class="stg-select" id="stg-theme-select" data-key="ff_theme" value="${get('ff_theme','dark')}">
            </div></label>
        </div>
        <div class="stg-group">
          <div class="stg-label">Layout</div>
          <label class="stg-row"><span>Sidebar width (px)</span>
            <div class="ff-stepper wide"><button class="ff-step-btn ff-step-dec" type="button">−</button><div class="ff-step-sep"></div><input type="number" class="stg-num" data-cssvar="--sidebar-w" min="140" max="400" value="${parseInt(getComputedStyle(document.documentElement).getPropertyValue('--sidebar-w'))||200}"><div class="ff-step-sep"></div><button class="ff-step-btn ff-step-inc" type="button">+</button></div></label>
          <label class="stg-row"><span>Preview panel width (px)</span>
            <div class="ff-stepper wide"><button class="ff-step-btn ff-step-dec" type="button">−</button><div class="ff-step-sep"></div><input type="number" class="stg-num" data-key="ff_preview_w" data-cssvar="--preview-w" min="160" max="600" value="${get('ff_preview_w','280')}"><div class="ff-step-sep"></div><button class="ff-step-btn ff-step-inc" type="button">+</button></div></label>
          <label class="stg-row"><span>Slideshow interval (seconds)</span>
            <div class="ff-stepper"><button class="ff-step-btn ff-step-dec" type="button">−</button><div class="ff-step-sep"></div><input type="number" class="stg-num" data-key="ff_ss_interval" min="1" max="30" value="${get('ff_ss_interval','3')}"><div class="ff-step-sep"></div><button class="ff-step-btn ff-step-inc" type="button">+</button></div></label>
        </div>
        <div class="stg-group">
          <div class="stg-label">Language</div>
          <label class="stg-row"><span>Interface language</span>
            <div class="ff-csel" style="min-width:160px">
              <button class="ff-csel-btn" type="button"><span class="ff-csel-label">${{en:'English',de:'Deutsch',es:'Español',fr:'Français',zh:'中文（简体）',ja:'日本語',ar:'العربية'}[get('ff_locale','en')]||'English'}</span><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
              <div class="ff-csel-popup">
                <div class="ff-csel-opt${get('ff_locale','en')==='en'?' active':''}" data-val="en">English</div>
                <div class="ff-csel-opt${get('ff_locale','en')==='de'?' active':''}" data-val="de">Deutsch</div>
                <div class="ff-csel-opt${get('ff_locale','en')==='es'?' active':''}" data-val="es">Español</div>
                <div class="ff-csel-opt${get('ff_locale','en')==='fr'?' active':''}" data-val="fr">Français</div>
                <div class="ff-csel-opt${get('ff_locale','en')==='zh'?' active':''}" data-val="zh">中文（简体）</div>
                <div class="ff-csel-opt${get('ff_locale','en')==='ja'?' active':''}" data-val="ja">日本語</div>
                <div class="ff-csel-opt${get('ff_locale','en')==='ar'?' active':''}" data-val="ar">العربية</div>
              </div>
              <input type="hidden" class="stg-select" data-key="ff_locale" value="${get('ff_locale','en')}">
            </div></label>
        </div>`; break;
      case 'search': c.innerHTML = `
        <div class="stg-group">
          ...
          <label class="stg-row"><span>Max results shown</span>
            <div class="ff-stepper wide"><button class="ff-step-btn ff-step-dec" type="button">−</button><div class="ff-step-sep"></div><input type="number" class="stg-num" data-key="ff_search_max" min="50" max="5000" value="${get('ff_search_max','500')}"><div class="ff-step-sep"></div><button class="ff-step-btn ff-step-inc" type="button">+</button></div></label>
        </div>`; break;
      case 'network': c.innerHTML = `
        <div class="stg-group">
          <div class="stg-label">SFTP</div>
          <label class="stg-row"><span>Connection timeout (seconds)</span>
            <div class="ff-stepper"><button class="ff-step-btn ff-step-dec" type="button">−</button><div class="ff-step-sep"></div><input type="number" class="stg-num" data-key="ff_sftp_timeout" min="5" max="120" value="${get('ff_sftp_timeout','30')}"><div class="ff-step-sep"></div><button class="ff-step-btn ff-step-inc" type="button">+</button></div></label>
        </div>
        <div class="stg-group">
      ...
      case 'advanced': c.innerHTML = `
        <div class="stg-group">
          ...
          <div class="stg-row"><span>Clear undo history (${state._undoStack?.length||0} steps on disk)</span>
            <button class="stg-btn" id="stg-clear-undo">Clear</button></div>
      ...
      case 'customisation': c.innerHTML = `
        <div class="stg-group">
          <div class="stg-label">Icon Theme</div>
          <div class="stg-row" style="flex-direction:column;align-items:flex-start;gap:6px;">
            <span style="font-size:12px;color:#9c9a92;">Load a folder of SVG files named after icon keys (e.g. folder.svg, file.svg). Partial themes are supported — unrecognised keys fall back to built-in icons.</span>
            <div style="display:flex;gap:8px;margin-top:4px;">
              <button class="stg-btn" id="stg-open-icon-theme">Open Icon Theme Picker</button>
            </div>
          </div>
        </div>
        <div class="stg-group">
    ...
    // Wire ff-stepper +/- buttons
    c.querySelectorAll('.ff-stepper').forEach(stepper => {
      ...
      stepper.querySelector('.ff-step-dec')?.addEventListener('click', () => {
        inp.value = clamp(+(inp.value||0) - 1); apply();
      });
      stepper.querySelector('.ff-step-inc')?.addEventListener('click', () => {
        inp.value = clamp(+(inp.value||0) + 1); apply();
      });
    });
    // Wire number inputs (stg-num — now inside ff-stepper)
    ...
    // Wire ff-csel custom selects (stg-select is now type=hidden inside .ff-csel)
    const _closeAllCsel = () => c.querySelectorAll('.ff-csel.open').forEach(el => el.classList.remove('open'));
    c.querySelectorAll('.ff-csel').forEach(csel => {
      ...
      csel.querySelectorAll('.ff-csel-opt').forEach(opt => {
        opt.addEventListener('click', e => {
          ...
          const hidden = csel.querySelector('.stg-select[data-key]');
          if (hidden) {
            hidden.value = val;
            const key = hidden.dataset.key;
            localStorage.setItem(key, val);
            if(key==='ff_icon_size'){ state.iconSize=+val; state.fontSize=Math.round(10+(state.iconSize-28)*(6/52)); applyScale();renderView(); }
            if(key==='ff_theme') applyTheme(val);
            if(key==='ff_locale') initI18n().then(()=>{ renderContent(); render(); });
          }
          _closeAllCsel();
        });
      });
    });
    document.addEventListener('click', _closeAllCsel, {once:true, capture:true});
...
function buildFileCtxMenu(entry){
  ...
  if(multi && sel.size===2){
    ...
    if(_bothDirs && sel.size === 2) items.push({label:'Merge folders…', action:'merge-folders', icon:I.folder});
  }
...
function setupDragDrop(el,entry,entries){
  ...
  el.addEventListener('dragstart',e=>{
    // Use hasp(path) not has(idx): in column view sel._e may point to a different column's
    // entries array, so has(idx) can resolve the wrong entry. Path-based check is always correct.
    const dragging=sel.size>0&&sel.hasp(entry.path)?getSelectedEntries():[entry];
...
function renderSidebar(){
  ...
  const drivesHtml = drives.length ? (()=>{
    ...
    const rows = drives.filter(d=>!HIDDEN_MOUNTS.has(d.path)).map(d=>{
      ...
      const bClr = {'usb':'#34d399','nvme':'#a78bfa','ssd':'#60a5fa','hdd':'#94a3b8','network':'#60a5fa','optical':'#f472b6','mtp':'#34d399'}[d.drive_type]||'#94a3b8';
    ...
    // r31 P4.2: MTP / Android & camera devices sidebar section
    const _mtpDevs = state._mtpDevices || [];
    const mtpDevicesHtml = _mtpDevs.length > 0
      ? '<div class="sb-section"><div class="sb-title">Devices</div>' +
        _mtpDevs.map(dev => {
          ...
          return `<div class="sb-item${mounted && state.activeSb===path?' active':''}" data-path="${mounted?escHtml(path):''}" data-mtp-id="${escHtml(dev.id)}">
            ...
            ${mounted
              ? `<button class="sb-eject-mtp" data-mtp-id="${escHtml(dev.id)}" title="Eject" style="background:none;border:none;color:#636368;cursor:pointer;padding:2px 4px;font-size:13px;">⏏</button>`
              : `<button class="sb-mount-mtp" data-mtp-id="${escHtml(dev.id)}" title="Mount" style="background:rgba(91,141,217,.15);border:1px solid rgba(91,141,217,.3);border-radius:4px;color:#5b8dd9;cursor:pointer;padding:1px 6px;font-size:10px;">Mount</button>`}
          </div>`;
        }).join('') + '</div>'
      : '';

    el.innerHTML='\n    ' + (recentSectionHtml) + '\n    <div class="sb-section">\n      <div class="sb-title">Favorites<div class="sb-size-ctrl"><button class="sb-size-btn" id="btn-organize-favs" title="Organize favorites"><svg viewBox="0 0 16 16" fill="currentColor" style="width:10px;height:10px"><path d="M2 4h12v2H2zm0 4h8v2H2zm0 4h10v2H2z"/></svg></button><button class="sb-size-btn" id="sb-size-dec" title="Smaller icons"><svg viewBox="0 0 10 2" fill="currentColor" style="width:10px;height:10px"><rect x="0" y="0" width="10" height="2" rx="1"/></svg></button><button class="sb-size-btn" id="sb-size-inc" title="Larger icons"><svg viewBox="0 0 10 10" fill="currentColor" style="width:10px;height:10px"><rect x="4" y="0" width="2" height="10" rx="1"/><rect x="0" y="4" width="10" height="2" rx="1"/></svg></button></div></div>\n      ' + (allFavs.map((f,i)=>`
  ...
  document.getElementById('btn-organize-favs')?.addEventListener('click',e=>{
  ...
  // r31 P4.2: MTP mount/eject buttons
  el.querySelectorAll('.sb-mount-mtp').forEach(btn => {
    btn.addEventListener('click', async e => {
      e.stopPropagation();
  ...
  document.querySelectorAll('.vbtn').forEach(btn=>{
    ...
    btn.addEventListener('click',async()=>{
      ...
      if(m==='column'){state.columns=[];await navigate(state.currentPath,0,false);}
        ...
        const hasFullMetadata = state.columns.length > 0 && 
          state.columns[state.columns.length-1].path === state.currentPath &&
          state.columns[state.columns.length-1].entries?.length > 0 &&
          'size' in state.columns[state.columns.length-1].entries[0];
        if(!state.columns.length||state.columns[state.columns.length-1].path!==state.currentPath||!hasFullMetadata){
          state.loading=true;render();
          try{
      ...
      if(savedSelIdx>=0){
        // FIX (RC2-R1 Bug 4): savedSelIdx is a global-sort index. In list view
        // rows are rendered in state.listSort order, so we must remap the index
        // to the list-sorted position by finding the saved path in that order.
        // For all other views sel._e == getVisibleEntries() so the index is valid.
        if(m==='list'){
          const _lsEntries=getVisibleEntries();
          const _savedPath=_lsEntries[savedSelIdx]?.path;
          if(_savedPath){
            const {col:lc,dir:ld}=state.listSort;
            const _ff=sortState?.foldersFirst??true;
            const _sorted=[..._lsEntries].sort((a,b)=>{
              if(_ff&&a.is_dir!==b.is_dir)return a.is_dir?-1:1;
              if(lc==='date')return ld*((a.modified||0)-(b.modified||0))||a.name.localeCompare(b.name);
              if(lc==='size')return ld*((a.size||0)-(b.size||0))||a.name.localeCompare(b.name);
              if(lc==='kind'){const ka=a.is_dir?'Folder':(a.extension||'');const kb=b.is_dir?'Folder':(b.extension||'');return ld*ka.localeCompare(kb)||a.name.localeCompare(b.name);}
              return ld*a.name.localeCompare(b.name,undefined,{sensitivity:'base'});
            });
            const _remapped=_sorted.findIndex(e=>e.path===_savedPath);
            sel._e=_sorted;
            const _idx=_remapped>=0?_remapped:0;
            sel.last=_idx; state.selIdx=_idx;
          }
        } else {
...
function _keysLabel(keys) {
  ...
  const k = keys.key;
  // Pretty-print special keys — ASCII only; avoid Unicode arrows/symbols
  // that may not be in the app's font (e.g. ⌫ U+232B renders as ⊠).
  const special = {
    ' ':         'Space',
    'Backspace': 'Bksp',
    'Delete':    'Del',
...
function setupKeyboard(){
  document.addEventListener('keydown',async e=>{
    ...
    if(isInput)return;
    // FIX (RC2-R1 Bug 1): in list view, rows are ordered by state.listSort which
    // differs from the global sortEntries() order used by getCurrentEntries().
    // renderListView() already updates sel._e to the list-sorted array, so reading
    // sel._e directly gives the correct order for keyboard navigation index math.
    // For all other views sel._e == getCurrentEntries() so this is always safe.
    const entries = (state.viewMode==='list' && sel._e.length) ? sel._e : getCurrentEntries();
    ...
    if(e.key==='ArrowDown'||e.key==='ArrowUp'||e.key==='ArrowLeft'||e.key==='ArrowRight'){
      // Vim mode: j/k/h/l maps to arrows
      const vimMap = {j:'ArrowDown',k:'ArrowUp',h:'ArrowLeft',l:'ArrowRight'};
      const mappedKey = vimMap[e.key] || e.key;
      if(vimMap[e.key]) e.preventDefault();
      let next=curIdx<0?0:curIdx;

      if(state.viewMode==='icon'){
        // Grid navigation using live column count
        const cols=state._iconCols||1;
        if(mappedKey==='ArrowRight') next=Math.min(curIdx+1,entries.length-1);
        else if(mappedKey==='ArrowLeft') next=Math.max(curIdx-1,0);
        else if(mappedKey==='ArrowDown') next=Math.min(curIdx+cols,entries.length-1);
        else if(mappedKey==='ArrowUp') next=Math.max(curIdx-cols,0);
      } else {
        if(mappedKey==='ArrowDown') next=Math.min(curIdx+1,entries.length-1);
        else if(mappedKey==='ArrowUp') next=Math.max(curIdx-1,0);
    ...
    if(e.key==='F4'){e.preventDefault();toggleTerminal();return;}
...
async function _dispatchKbAction(e, kb, noInputOnly, ctx) {
  ...
  for (const [id, binding] of Object.entries(kb)) {
    ...
    switch(id) {
      ...
      case 'clipboard-history':_showClipboardHistoryPanel(); break;
      ...
      case 'compare-files': {
        const _files=getSelectedEntries().filter(en=>!en.is_dir);
        if(_files.length===2){ _showFileDiff(_files[0].path,_files[1].path); }
        else { showToast(t('toast.select_2_files'),'info'); }
        break;
      }
...
async function init(){
  ...
  // r28 P2.1: Wire undo module deps
  initUndoDeps({ state, showToast, t, refreshColumns, escHtml: _escHtml });
  // r29 P2.1: Wire settings module deps
  initSettingsDeps({ showToast, t });
  // r29 P2.1: Wire network and plugin module deps
  initNetworkDeps({ showToast, t, renderSidebar });
  initPluginDeps({ showToast, t, refreshCurrent, sbProgress: _sbProgress, state });
  ...
  // r28 P2.3: also runs one-shot migration from legacy tags.json → SQLite.
  setTimeout(async () => {
    try {
      const migrated = await invoke('migrate_tags_to_sqlite');
      if (migrated > 0) FF.log('TAG_MIGRATE', { migrated });
    } catch (e) { /* non-fatal — v1 JSON store may not exist */ }
    try {
      const removed = await invoke('cleanup_tag_db');
      if (removed > 0) {
  ...
  initQuickLook().catch(()=>{});
  // r29 P2.1: Load persistent settings from Rust before sidebar renders
  await loadPersistentSettings();
  patchLocalStorage();
  // r30 P3.1: Restore undo history from disk (fire after settings load)
  restoreUndoHistory().catch(() => {});
  await loadSidebar();
  ...
  // r31 P4.2: Poll for MTP devices every 5 seconds (hot-plug detection fallback)
  let _mtpDevices = [];
  const _pollMtp = async () => {
    try {
      const devs = await invoke('detect_mtp_devices');
      if (JSON.stringify(devs) !== JSON.stringify(_mtpDevices)) {
        _mtpDevices = devs;
        state._mtpDevices = devs;
        renderSidebar();
      }
    } catch (_) {}
  };
  _pollMtp();
  setInterval(_pollMtp, 5000);
...
let activePane = 0;
let dualPaneEnabled = localStorage.getItem('ff_dual_pane') === '1';
...
function toggleDualPane() {
  ...
  localStorage.setItem('ff_dual_pane', dualPaneEnabled ? '1' : '0');
...
function swapPanes() {
  ...
  const tmpPath = panes[0].path;
  const tmpSel = panes[0].selIdx;
  panes[0].path = panes[1].path;
  panes[0].selIdx = panes[1].selIdx;
  panes[1].path = tmpPath;
  panes[1].selIdx = tmpSel;
  navigate(panes[activePane].path || state.currentPath, 0, false);
}
...
window._editPlugin = (idx) => {
  const _plugins = getPlugins(); const p = idx !== null ? _plugins[idx] : {name:'',icon:'▶',match:'*',command:'',multi:false,confirm:false,notify:true};
...
window._savePlugin = async (idx) => {
  ...
  if (idx!==null) getPlugins()[idx]=plugin; else getPlugins().push(plugin);
...
// r29 P2.1: _settingsLoaded moved to settings.js

// r29 P2.1: localStorage patch moved to settings.js → patchLocalStorage()
