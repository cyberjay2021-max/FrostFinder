...
// r28 P2.2 Stage 1: Tag storage extracted to tags.rs module.
// All symbols are re-exported so existing callers compile unchanged.
pub mod search;
pub use search::{
    deep_search, index_apply_event, index_home_dir, search_advanced, search_files,
    search_index_query, search_index_store, DeepSearchResult, IndexEntry, IndexSearchResult,
    SearchResultV2,
};

pub mod trash;
pub use trash::{
    check_trash_restore_conflicts, trash_item_count, trash_items, trash_list,
    trash_restore_with_resolution, RestoreInstruction, TrashConflict, TrashItem,
};
...
pub use tags::{
    audit_tag_db, cleanup_tag_db, db_read_tags, db_write_tags, get_all_tags, get_all_tags_v2,
    get_file_tags, get_file_tags_v2, get_tag_palette, get_tags_with_colors,
    get_tags_with_colors_v2, load_tag_colors, load_tags_db, migrate_tag_path,
    migrate_tags_to_sqlite, save_tag_colors, save_tags_db, scan_xattr_tags, search_by_tag,
    search_by_tag_v2, set_file_tags, set_file_tags_v2, set_tag_color, set_tag_color_v2,
    tag_colors_db_path, tag_db, tag_db_maybe_checkpoint, tag_db_stats, tags_db_path,
    xattr_read_tags, xattr_write_tags, FileTag,
};
...
fn check_optional_deps() -> std::collections::HashMap<String, bool> {
    ...
    // r32 P5.3: rclone RC daemon availability
    deps.insert("rclone_rc".to_string(), which("rclone"));
    deps
}
...
fn parse_mounts() -> Vec<(String, String, String)> {
    ...
    if let Ok(c) = fs::read_to_string("/proc/mounts") {
        for line in c.lines() {
            ...
            if p.len() >= 3 {
                results.push((p[0].to_string(), p[1].to_string(), p[2].to_string()));
            }
        }
    }
    results
...
#[cfg(not(unix))]
fn _get_disk_space_impl(_path: &str) -> (u64, u64) {
    (0, 0)
}
...
fn is_usb_device(dev: &str) -> bool {
    ...
    let is_usb_uevent = fs::read_to_string(format!("/sys/block/{}/device/uevent", base))
        .map(|s| s.to_uppercase().contains("USB"))
        .unwrap_or(false);
    let is_usb_subsystem = fs::read_link(format!("/sys/block/{}/device/subsystem", base))
        .ok()
        .and_then(|p| {
            p.file_name()
                .map(|n| n.to_string_lossy().to_lowercase().contains("usb"))
        })
        .unwrap_or(false);
...
fn get_volume_label(device: &str) -> Option<String> {
    ...
    for entry in fs::read_dir(by_label).ok()?.filter_map(|e| e.ok()) {
        if let Ok(target) = fs::read_link(entry.path()) {
            let resolved =
                fs::canonicalize(by_label.join(&target)).unwrap_or_else(|_| by_label.join(&target));
...
fn classify_drive(device: &str, mountpoint: &str, fstype: &str) -> Option<DriveInfo> {
    ...
    if mountpoint.starts_with("/proc")
        ...
        || mountpoint.starts_with("/run/credentials")
        || mountpoint.starts_with("/run/lock")
        || mountpoint.starts_with("/run/systemd")
    {
        return None;
    }
    // Filter OS-internal system mountpoints that users don't navigate to
    let exact_hide = [
        "/boot", "/tmp", "/var/log", "/srv", "/cache", "/log", "/root",
        "/", // Root Disk — hide, users navigate via Home
    ];
    if exact_hide.contains(&mountpoint) {
        return None;
    }
    // Hide anything whose last path component is "cache" or "tmp" or "log"
    let last_seg = mountpoint
        .split('/')
        .filter(|s| !s.is_empty())
        .next_back()
        .unwrap_or("");
    if ["cache", "tmp", "log", "lost+found", "proc", "sys"].contains(&last_seg) {
        return None;
    }
    // Always allow /run/media and /media (standard Linux USB mount points)
    if !device.starts_with('/') && !device.starts_with("//") {
        return None;
    }
    ...
    let drive_type = if fstype.starts_with("nfs") || fstype == "cifs" || fstype == "fuse.sshfs" {
        "network"
    } else if dev_short.contains("sr") || dev_short.contains("cdrom") {
        "optical"
    }
    ...
    else if dev_short.starts_with("sd")
        || dev_short.starts_with("vd")
        || dev_short.starts_with("hd")
        || dev_short.starts_with("nvme")
        || dev_short.starts_with("mmcblk")
    {
        if is_usb_device(dev_short) {
            "usb"
        } else if dev_short.starts_with("nvme") {
            "nvme"
        } else if is_rotational(dev_short) {
            "hdd"
        } else {
            "ssd"
        }
    ...
    } else if mountpoint.starts_with("/run/media") || mountpoint.starts_with("/media") {
        "usb"
    }
    ...
    let name = if mountpoint == "/" {
        ...
        get_volume_label(device).unwrap_or_else(|| {
            if drive_type == "usb" {
                // Try lsblk for label
...
#[derive(Debug, Serialize, Deserialize)]
pub struct DirectoryListingFast {
    pub path: String,
    pub entries: Vec<FileEntryFast>,
    pub parent: Option<String>,
}
...
async fn preload_dir(path: String) {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let entries: Vec<FileEntryFast> = rd
            .filter_map(|e| e.ok())
            .map(|entry| {
...
async fn list_directory_streamed(
    window: Window,
    path: String,
    request_id: u32,
) -> Result<(), String> {
...
fn list_directory_chunk(paths: Vec<String>) -> Vec<FileEntry> {
    ...
    paths
        .par_iter()
        .filter_map(|p| build_file_entry(Path::new(p)))
        .collect()
}
...
#[tauri::command]
async fn list_directory_full_streamed(
    window: Window,
    path: String,
    request_id: u32,
) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        if !dir_path.is_dir() {
            ...
            let is_sym_dir =
                sym.map(|m| m.file_type().is_symlink()).unwrap_or(false) && dir_path.is_dir();
            if !is_sym_dir {
                return Err(format!("PERMISSION_DENIED:{}", path));
            }
        }
        let raw: Vec<_> = fs::read_dir(dir_path)
            .map_err(|e| {
                if e.kind() == std::io::ErrorKind::PermissionDenied {
                    format!("PERMISSION_DENIED:{}", path)
                } else {
...
fn lsblk_unmounted_all(
    ...
    let out = match std::process::Command::new("lsblk")
        .args([
            ...
            "-b",
            "-o",
            "NAME,LABEL,MOUNTPOINT,RM,HOTPLUG,TYPE,FSTYPE,PATH,TRAN",
        ])
        .output()
    {
    ...
    if let Some(devs) = json["blockdevices"].as_array() {
        for disk in devs {
            ...
            let disk_rm =
                disk["rm"].as_bool().unwrap_or(false) || disk["hotplug"].as_bool().unwrap_or(false);
            ...
            for part in candidates {
                ...
                if mounted_devs.contains(dev_path) {
                    continue;
                }
                ...
                if part_type == "loop" || part_type == "swap" {
                    continue;
                }
                ...
                let drive_type = {
                    ...
                    if rm
                        || disk_tran == "usb"
                        || part_name.starts_with("sd")
                            && is_usb_device(part_name.split('/').next_back().unwrap_or(part_name))
                    {
                        "usb"
                    } else if disk_tran == "nvme"
                        || part_name.starts_with("nvme")
                        || disk_name.starts_with("nvme")
                    {
                        "nvme"
...
fn collect_drives_with_unmounted() -> Vec<DriveInfo> {
    ...
    let mut drives: Vec<DriveInfo> = mounts
        .iter()
        .filter_map(|(dev, mnt, fs)| {
            mounted_devs.insert(dev.clone());
            let info = classify_drive(dev, mnt, fs)?;
            if !seen_mnt.insert(mnt.clone()) {
                return None;
            }
            Some(info)
        })
...
fn get_sidebar_data() -> SidebarData {
    ...
    let raw_favs = [
        ...
        (
            "Videos",
            home.join("Videos").to_string_lossy().to_string(),
            "video",
        ),
        ("Trash", trash_path.to_string_lossy().to_string(), "trash"),
    ];
    let favorites = raw_favs
        .iter()
        .map(|(n, p, i)| SidebarFavorite {
            name: n.to_string(),
            path: p.to_string(),
            icon: i.to_string(),
            exists: if *i == "trash" {
...
fn get_drives_platform() -> Vec<DriveInfo> {
    ...
    for line in text.lines().skip(2) {
        ...
        if device_id.is_empty() || drive_type == 5 {
            continue;
        } // skip CD/DVD
...
fn mount_drive(device: String) -> Result<String, String> {
    ...
    {
        let _ = device;
        return Err("mount_drive is only supported on Linux.".into());
    }
    #[cfg(target_os = "linux")]
    {
...
fn unlock_and_mount_encrypted(device: String, passphrase: String) -> Result<String, String> {
    #[cfg(not(target_os = "linux"))]
    {
        let _ = (device, passphrase);
        return Err("Encrypted drive unlock is only supported on Linux.".into());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        Err(String::from_utf8_lossy(&mount_out.stderr)
            .trim()
            .to_string())
...
fn get_file_preview(path: String) -> Result<FilePreview, String> {
    ...
    let is_text = text_exts.contains(&ext.as_str())
        || fname_lower == "makefile"
        || fname_lower == "dockerfile"
        || fname_lower == ".gitignore"
        || fname_lower == ".env";
    ...
    match ext.as_str() {
        ...
        "pptx" | "ppt" => {
            if let Some(text) = pptx_to_text(p) {
                let line_count = text.lines().count();
                return Ok(FilePreview {
                    path,
                    content: Some(text),
        ...
        "odt" | "odp" | "ods" | "odg" => {
            if let Some(text) = odt_to_text(p) {
                let line_count = text.lines().count();
                return Ok(FilePreview {
                    path,
                    content: Some(text),
    ...
    if is_text {
        ...
        if !raw.is_empty() && raw[..raw.len().min(4096)].contains(&0) {
            return Ok(FilePreview {
                path,
                content: None,
                image_base64: None,
        ...
        let content = if raw.len() > limit {
            format!(
                "{}\n\n[... truncated at 256 KB, {} bytes total]",
                content, size
            )
        } else {
            content
...
fn make_thumbnail(path: &str) -> Result<PathBuf, String> {
    ...
    let mtime = meta
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    if let Some(cached) = thumb_cache_get(path, mtime) {
        return Ok(cached);
    }
    ...
    if ext == "heic" || ext == "heif" {
        ...
        let out = std::process::Command::new("ffmpeg")
            .args([
                ...
                path,
                "-vf",
                "scale=256:256:force_original_aspect_ratio=decrease,format=yuv420p",
                "-frames:v",
                "1",
                "-pix_fmt",
                "yuvj420p",
    ...
    const VIDEO_EXTS: &[&str] = &[
        "mp4", "mkv", "webm", "avi", "mov", "ogv", "m4v", "flv", "ts", "wmv", "3gp",
    ];
    if VIDEO_EXTS.contains(&ext.as_str()) {
        ...
        let ffmpeg_result = std::process::Command::new("ffmpeg")
            .args([
                ...
                "-vf",
                "scale=256:-1", // resize to 256px wide, keep AR
                "-f",
                "image2pipe",
                "-vcodec",
                "mjpeg",
...
async fn empty_trash_stream(window: tauri::Window) -> Result<usize, String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let total: usize = [&trash_files, &trash_info]
            .iter()
        ...
        if b == b'\\' {
                ...
                && (bytes[i + 1].is_ascii_digit() || bytes[i + 1] == b'-')
            {
                ...
                if let Ok(n) = raw[start2..i].parse::<i32>() {
                    if let Some(ch) = char::from_u32(n.unsigned_abs()) {
                        out.push(ch);
                    }
                }
...
/// r31 P4.1: Extract slide text from a .pptx file (Office Open XML Presentation).
/// .pptx is a ZIP containing ppt/slides/slide*.xml — we read all slide text nodes.
fn pptx_to_text(path: &Path) -> Option<String> {
    use std::io::Read;
    ...
    for name in &names {
        ...
        let mut pos = 0;
        while let Some(s) = xml[pos..].find("<a:t>") {
            let start = pos + s + 5;
            if let Some(e) = xml[start..].find("</a:t>") {
                let text = xml[start..start + e].trim().to_string();
                if !text.is_empty() {
                    texts.push(text);
                }
                pos = start + e + 6;
...
/// r31 P4.1: Extract text from an .odt/.odp/.ods file (ODF — also a ZIP with content.xml).
fn odt_to_text(path: &Path) -> Option<String> {
    use std::io::Read;
    let file = std::fs::File::open(path).ok()?;
    let mut archive = zip::ZipArchive::new(file).ok()?;
    ...
    let mut texts: Vec<String> = Vec::new();
    for tag in &["<text:p", "<text:span"] {
        let mut p = 0;
        while let Some(s) = xml[p..].find(tag) {
            let tag_start = p + s;
            let close_start = xml[tag_start..].find('>')?;
            let text_start = tag_start + close_start + 1;
            let end_tag = &format!("</{}", &tag[1..]);
            if let Some(e) = xml[text_start..].find(end_tag.as_str()) {
                let text = xml[text_start..text_start + e].trim().to_string();
                if !text.is_empty() && !text.starts_with('<') {
                    texts.push(text);
                }
                p = text_start + e + end_tag.len() + 1;
            } else {
                p = text_start;
            }
        }
...
/// r31 P4.1: Try converting an office file to PDF via LibreOffice headless.
/// Caches the output in ~/.cache/frostfinder/ql/<hash>.pdf.
/// Returns the PDF path on success, None if LibreOffice is absent or fails.
fn office_to_pdf_cached(path: &Path) -> Option<std::path::PathBuf> {
    // Check LibreOffice availability
    let lo_bin = ["libreoffice", "soffice"].iter().find(|&&b| which(b))?;
    // Build a stable cache key from path + mtime
    ...
    let pdf_path = cache_dir.join(format!("{}.pdf", hash));
    if pdf_path.exists() {
        return Some(pdf_path);
    }
    // Convert — LibreOffice writes to --outdir
    let out = std::process::Command::new(lo_bin)
        .args([
            "--headless",
            "--convert-to",
            "pdf",
            "--outdir",
            cache_dir.to_str().unwrap_or("/tmp"),
    ...
    // LibreOffice names the output after the stem, e.g. report.pdf
    let stem = path.file_stem()?.to_string_lossy();
    let lo_out = cache_dir.join(format!("{}.pdf", stem));
    if lo_out.exists() {
        // Rename to our deterministic hash name
        std::fs::rename(&lo_out, &pdf_path).ok()?;
        Some(pdf_path)
    } else {
        None
    }
...
/// r31 P4.1: Get office file preview — tries PDF conversion first (LibreOffice),
/// falls back to text extraction, degrades to None gracefully.
#[tauri::command]
fn get_office_preview(path: String) -> serde_json::Value {
    ...
    let ext = p
        ...
        .unwrap_or("")
        .to_lowercase();
    // Try LibreOffice PDF conversion
    if let Some(pdf) = office_to_pdf_cached(p) {
    ...
    let text = match ext.as_str() {
        "pptx" | "ppt" => pptx_to_text(p),
        "odt" | "odp" | "ods" | "odg" => odt_to_text(p),
        _ => None,
    };
    if let Some(t) = text {
        return serde_json::json!({"mode": "text", "content": t});
    }
    // Signal that LibreOffice is needed
    serde_json::json!({"mode": "install_nudge", "ext": ext})
}
...
fn epub_to_text(path: &Path) -> Option<String> {
    ...
    let spine_hrefs: Vec<String> = if !opf_path.is_empty() {
        if let Ok(mut entry) = archive.by_name(&opf_path) {
            ...
            let mut id_href: std::collections::HashMap<String, String> =
    ...
    for name in &file_names {
        ...
        let xml = {
            ...
            if archive
                .by_name(name)
                .map(|mut e| e.read_to_string(&mut xml))
                .is_err()
            {
                continue;
            }
...
fn mobi_to_text(path: &Path) -> Option<String> {
    ...
    let text_data: &[u8] = {
        ...
        for i in 0..data.len().saturating_sub(6) {
            if &data[i..i.min(i + 5)] == b"<html"
                || &data[i..i.min(i + 5)] == b"<body"
                || &data[i..i.min(i + 2)] == b"<p"
            {
    ...
    for ch in raw.chars() {
        match ch {
            ...
            _ => {
                if !in_tag && ch.is_ascii_graphic() || ch == ' ' || ch == '\n' || !ch.is_ascii() {
                    if !in_tag {
                        out.push(ch);
                    }
                } else {
    ...
    if result.trim().len() < 50 {
        return None;
    } // Too little real text — likely not parseable
    Some(result)
}
...
async fn find_duplicates(
    ...
    let _ = window.emit(
        "duplicates-progress",
        serde_json::json!({"phase":"scanning","done":0,"total":0}),
    );
...
async fn delete_items_stream(
    ...
    let _ = window.emit(
        "delete-progress",
        serde_json::json!({"name":"","done":0,"total":total,"finished":false}),
    );
    for (idx, path) in paths.iter().enumerate() {
        let nm = Path::new(path)
            .file_name()
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_default();
        let _ = window.emit(
            "delete-progress",
            serde_json::json!({"name":&nm,"done":idx as u64,"total":total,"finished":false}),
        );
        ...
        if let Err(e) = tauri::async_runtime::spawn_blocking(move || -> Result<(), String> {
            ...
            fs::create_dir_all(&tf).map_err(|e| e.to_string())?;
            fs::create_dir_all(&ti).map_err(|e| e.to_string())?;
            let name = p
                .file_name()
                .ok_or("no name")?
                .to_string_lossy()
    ...
    let _ = window.emit(
        "delete-progress",
        serde_json::json!({"name":"","done":total,"total":total,"finished":true}),
    );
    Ok(())
}
...
async fn install_font(path: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        if dest.exists() {
            return Err("Font is already installed.".into());
        }
...
fn is_font_installed(filename: String) -> bool {
    dirs::home_dir()
        .map(|h| h.join(".local/share/fonts").join(&filename).exists())
        .unwrap_or(false)
}

#[tauri::command]
fn create_directory(path: String, name: String) -> Result<String, String> {
    let d = Path::new(&path).join(&name);
    if d.exists() {
...
fn create_file_cmd(path: String, name: String) -> Result<String, String> {
    let f = Path::new(&path).join(&name);
    if f.exists() {
        return Err(format!("'{}' exists", name));
    }
    fs::write(&f, "").map_err(|e| e.to_string())?;
...
fn copy_file_sync(src: String, dest_dir: String) -> Result<String, String> {
    ...
    let name = src_p.file_name().ok_or("No filename")?;
    ...
    if dest.exists() {
        ...
        let mut n = 2;
        loop {
            dest = Path::new(&dest_dir).join(format!("{} copy {}{}", stem, n, ext));
            if !dest.exists() {
                break;
            }
            n += 1;
...
fn move_file_sync(src: String, dest_dir: String) -> Result<String, String> {
    ...
    if fs::rename(src_p, &dest).is_ok() {
        return Ok(dest.to_string_lossy().to_string());
    }
    if src_p.is_dir() {
        copy_dir_recursive(src_p, &dest)?;
        fs::remove_dir_all(src_p).map_err(|e| e.to_string())?;
    } else {
        fs::copy(src_p, &dest).map_err(|e| e.to_string())?;
        fs::remove_file(src_p).map_err(|e| e.to_string())?;
    }
    Ok(dest.to_string_lossy().to_string())
}
...
async fn compress_files(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        _compress_files_sync(window, paths, output_path, compression_level, password)
    })
    .await
    .map_err(|e| format!("thread error: {}", e))?
}

fn _compress_files_sync(
    window: tauri::Window,
    paths: Vec<String>,
    output_path: String,
    compression_level: Option<u8>,
    ...
    // Build options - with AES encryption if password provided
    let mut file_count = 0usize;

    // Process with or without password - use separate code paths for clarity
    if let Some(ref pw) = password {
        // AES encrypted compression - use with_aes_encryption method
        for input_path in &paths {
            let base = Path::new(input_path);
        ...
        // No password - regular compression
        let options = {
            let base = FileOptions::<()>::default()
                .compression_method(method)
                .unix_permissions(0o755);
            if let Some(lvl) = level_opt {
                base.compression_level(Some(lvl))
            } else {
                base
            }
        };
...
async fn extract_archive(
    ...
    {
        return Err(
            "An extraction is already in progress — please wait for it to finish.".to_string(),
        );
    }
...
fn delete_items(paths: Vec<String>) -> Result<(), String> {
    for path in &paths {
        ...
        if !p.exists() {
            continue;
        } // already gone — not an error for undo
        if p.is_dir() {
...
#[tauri::command]
fn run_command(cmd: String, cwd: String) -> Result<(String, String), String> {
...
fn open_in_editor(path: String) -> Result<(), String> {
    ...
    {
        let editors = [
            "code", "codium", "kate", "gedit", "mousepad", "pluma", "micro",
        ];
        for &e in &editors {
            if std::process::Command::new("which")
                .arg(e)
...
#[tauri::command]
fn window_maximize(window: tauri::WebviewWindow) -> Result<bool, String> {
...
#[tauri::command]
fn window_set_fullscreen(window: tauri::WebviewWindow, fullscreen: bool) -> Result<(), String> {
...
fn start_media_server() -> u16 {
    ...
    if let Ok(f) = std::fs::File::open(&port_file) {
        ...
        if let (Some(Ok(port_str)), Some(Ok(pid_str))) = (lines.next(), lines.next()) {
            if let (Ok(existing_port), Ok(existing_pid)) = (
...
fn handle_media_request(stream: &mut std::net::TcpStream) {
    ...
    loop {
        ...
        if line.to_lowercase().starts_with("range:") {
            range_hdr = Some(line["range:".len()..].trim().to_string());
        }
    }
...
";
    ...
    let mut buf = vec![0u8; 262144]; // 256 KB — reduces syscall overhead for large 4K files
    while remaining > 0 {
...
fn get_native_window_handle(window: tauri::WebviewWindow) -> Result<NativeWindowHandle, String> {
    ...
    match raw {
        RawWindowHandle::Xlib(w) => Ok(NativeWindowHandle {
            backend: "x11".into(),
            handle: w.window as i64,
        }),
        RawWindowHandle::Xcb(w) => Ok(NativeWindowHandle {
            backend: "x11".into(),
            handle: w.window.get() as i64,
        }),
        RawWindowHandle::Wayland(w) => Ok(NativeWindowHandle {
            backend: "wayland".into(),
            handle: w.surface.as_ptr() as i64,
        }),
...
fn mpv_open_external(
    ...
    let child = std::process::Command::new("mpv")
        ...
        .map_err(|e| {
            format!(
                "mpv not found — install with: sudo pacman -S mpv
({e})"
            )
        })?;
...
fn mpv_is_running() -> bool {
    let mut lock = match mpv_child().lock() {
        Ok(l) => l,
        Err(_) => return false,
    };
    match lock.as_mut() {
...
fn url_decode(s: &str) -> String {
    ...
    let mut raw: Vec<u8> = Vec::with_capacity(bytes_in.len());
    let mut i = 0;
...
fn diff_files(path_a: String, path_b: String) -> Result<DiffResult, String> {
    ...
    let out = std::process::Command::new("diff")
        .args([
            "-u", "--label", &path_a, "--label", &path_b, &path_a, &path_b,
        ])
        .output()
...
fn read_svg_icon(path: String) -> Result<String, String> {
    let p = Path::new(&path);
    if !p.exists() {
        return Err(format!("Icon not found: {}", path));
    }
    let ext = p
        .extension()
        .map(|e| e.to_string_lossy().to_lowercase())
...
fn mount_iso(path: String) -> Result<String, String> {
    ...
    {
        let _ = path;
        return Err("ISO mounting is only supported on Linux.".into());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let ext = p
            ...
            .unwrap_or_default();
        if ext != "iso" {
            return Err("Only .iso files can be mounted.".into());
        }
        ...
        let loop_dev_opt = setup_combined.split_whitespace().find_map(|w| {
            let clean = w.trim_end_matches(['.', ',', ';', '\'', '"']);
            if clean.starts_with("/dev/loop") && clean.len() > 9 {
        ...
        let loop_dev = if let Some(dev) = loop_dev_opt {
            ...
            ls_out
                ...
                .ok_or_else(|| {
                    format!("Could not find loop device. udisksctl output: {setup_combined}")
                })?
        };
...
fn unmount_iso(loop_dev: String) -> Result<(), String> {
    ...
    {
        let _ = loop_dev;
        return Err("ISO unmounting is only supported on Linux.".into());
    }
...
fn mount_smb(
    ...
    #[cfg(not(target_os = "linux"))]
    {
        let _ = (server, share, username, password);
        return Err("SMB mounting is only supported on Linux.".into());
    }
...
fn unmount_smb(server: String, share: String) -> Result<(), String> {
    ...
    {
        let _ = (server, share);
        return Err("SMB unmounting is only supported on Linux.".into());
    }
...
fn list_smb_shares(
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        return Err("SMB listing is only supported on Linux.".into());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        let out = std::process::Command::new("lsblk")
            .args([
                ...
                "-b",
                "-o",
                "NAME,SIZE,TYPE,RM,HOTPLUG,VENDOR,MODEL,LABEL,MOUNTPOINT",
            ])
            .output();
        let Ok(out) = out else {
            return Vec::new();
        ...
        for line in text.lines() {
                ...
                if depth == 2 {
                    ...
                    if is_removable && is_disk && not_system && !cur_name.is_empty() && cur_size > 0
                    {
                        let label_parts: Vec<&str> =
                            [cur_vendor.as_str(), cur_model.as_str(), cur_label.as_str()]
                                .iter()
                                .filter(|s| !s.is_empty())
                                .copied()
                                .collect();
                        let label = if label_parts.is_empty() {
                            ...
                            label_parts.join(" ").trim().to_string()
                        };
            ...
            if depth == 2 {
                ...
                } else if t.contains(r#""rm": true"#) {
                    cur_rm = true;
                } else if t.contains(r#""hotplug": true"#) {
                    cur_hotplug = true;
...
async fn write_iso_to_usb(
    ...
    #[cfg(not(target_os = "linux"))]
    {
        let _ = (window, iso_path, device);
        return Err("ISO write is only supported on Linux.".into());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        {
            return Err(format!(
                "{device} looks like a partition — use the whole disk (e.g. /dev/sdb)."
            ));
        }

        // Refuse to write to the system root disk
        let root_dev = fs::read_to_string("/proc/mounts")
            .unwrap_or_default()
            .lines()
            .find(|l| l.split_whitespace().nth(1) == Some("/"))
            .and_then(|l| l.split_whitespace().next().map(|s| s.to_string()))
            .unwrap_or_default();
        let root_base: String = root_dev
            .trim_end_matches(|c: char| c.is_ascii_digit())
            .to_string();
        if root_base == device {
            return Err(format!(
                "Refusing to overwrite {device} — it is the system root device."
            ));
        }
...
fn mount_dmg(path: String) -> Result<String, String> {
    ...
    {
        let _ = path;
        return Err("DMG mounting is only supported on Linux.".into());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let ext = p
            ...
            .unwrap_or_default();
        if ext != "dmg" {
            return Err("Only .dmg files can be mounted here.".into());
        }
...
fn unmount_dmg(loop_dev: String) -> Result<(), String> {
    ...
    {
        let _ = loop_dev;
        return Err("DMG unmounting is only supported on Linux.".into());
    }
...
fn mount_webdav(
    ...
    #[cfg(not(target_os = "linux"))]
    {
        let _ = (name, url, username, password);
        return Err("WebDAV mounting is only supported on Linux.".into());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let nonword_re =
            NONWORD_RE.get_or_init(|| regex::Regex::new(r"[^\w]").expect("static regex"));
        ...
        let output = cmd
            .output()
            .map_err(|e| format!("mount.davfs failed: {}", e))?;
...
fn unmount_cloud(cloud_id: String) -> Result<(), String> {
    ...
    {
        let _ = cloud_id;
        return Err("Cloud unmounting is only supported on Linux.".into());
    }
...
fn scan_icon_folder(folder_path: String) -> Result<Vec<IconHit>, String> {
    ...
    fn walk(
        ...
        if depth > 8 {
            return;
        } // guard against runaway symlink trees
...
fn add_cloud_provider(provider: String, label: String) -> Result<String, String> {
    #[cfg(not(target_os = "linux"))]
    {
        ...
        return Err("Cloud provider auth is only supported on Linux in this version.".into());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if !status.success() {
            return Err(format!(
                "rclone authorisation failed (exit {:?}). Check that a browser is available.",
                status.code()
            ));
        }
...
fn mount_cloud_provider(remote_name: String) -> Result<String, String> {
    ...
    {
        let _ = remote_name;
        return Err("Cloud mounting is only supported on Linux.".into());
    }
...
fn remove_cloud_provider(remote_name: String) -> Result<(), String> {
    ...
    if !status.success() {
        return Err(format!(
            "rclone config delete failed (exit {:?})",
            status.code()
        ));
    }
    Ok(())
...
fn restore_cloud_mounts() -> Vec<String> {
    let Ok(remotes) = list_rclone_remotes() else {
        return Vec::new();
    };
...
fn git_status_cache(
) -> &'static std::sync::Mutex<std::collections::HashMap<String, (GitStatus, u64)>> {
    static GIT_STATUS_CACHE: std::sync::OnceLock<
        std::sync::Mutex<std::collections::HashMap<String, (GitStatus, u64)>>,
...
fn create_vault(
    ...
    #[cfg(not(target_os = "linux"))]
    {
        let _ = (name, encrypted_dir, password);
        return Err("Encrypted vaults require Linux.".into());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let id = uuid_v4();
        let mount_point = vault_mount_base().join(&id).to_string_lossy().to_string();
...
fn is_fuse_path(path: &str) -> bool {
    ...
    let mut entries: Vec<(String, String)> = mounts
        ...
        .filter_map(|line| {
            ...
            if cols.len() >= 3 {
                Some((cols[1].to_string(), cols[2].to_string()))
            } else {
                None
            }
        })
...
fn append_error_log(message: String) -> Result<(), String> {
    ...
    let sanitised = {
        ...
        if home.is_empty() {
            ...
            message
                .replace(&format!("{}/", home.trim_end_matches('/')), "~/")
                .replace(&home, "~")
        }
    };
...
fn main() {
    ...
    // ═══════════════════════════════════════════════════════════════════════════════
    // Phase 6 — Native EXIF metadata (no exiftool dependency)
    // ═══════════════════════════════════════════════════════════════════════════════
    ...
    #[tauri::command]
    fn get_exif_tags(path: String) -> Result<ExifData, String> {
        ...
        let get_rational = |tag: exif::Tag| -> Option<f64> {
            exif.get_field(tag, exif::In::PRIMARY).and_then(|f| {
                if let exif::Value::Rational(ref v) = f.value {
                    if !v.is_empty() {
                        return Some(v[0].num as f64 / v[0].denom as f64);
                    }
                }
                None
            })
        ...
        let get_uint = |tag: exif::Tag| -> Option<u32> {
            exif.get_field(tag, exif::In::PRIMARY).and_then(|f| {
        ...
        Ok(ExifData {
            date_time_original: get_str(exif::Tag::DateTimeOriginal),
    ...
    #[tauri::command]
    fn write_exif_tags(path: String, fields: Vec<[String; 2]>) -> Result<(), String> {
        if fields.is_empty() {
            return Ok(());
        }
        ...
        match ext.as_str() {
            "jpg" | "jpeg" => write_jpeg_exif(&path, fields),
            "png" => write_png_exif(&path, fields),
            _ => Err(format!("EXIF writing not supported for .{} files", ext)),
        }
    }

    fn write_jpeg_exif(path: &str, fields: Vec<[String; 2]>) -> Result<(), String> {
        use std::io::Read;
        ...
        exif_data.extend_from_slice(&42u16.to_le_bytes()); // Magic number
        exif_data.extend_from_slice(&8u32.to_le_bytes()); // Offset to first IFD

        let mut ifd_data = Vec::new();
        let mut tag_count: u16 = 0;

        for [tag_name, value] in &fields {
            let tag_num: u16 = match tag_name.as_str() {
            ...
            match tag_name.as_str() {
                "DateTimeOriginal" => {
                    ...
                    tag_data = s.as_bytes().to_vec();
                    tag_type = 2; // ASCII
                    tag_count_val = tag_data.len() as u32 + 1;
                }
                "Make" | "Model" => {
                    let s = value.trim();
                    if s.is_empty() {
            ...
            ifd_data.extend_from_slice(&tag_num.to_le_bytes());
            ifd_data.extend_from_slice(&tag_type.to_le_bytes());
            ifd_data.extend_from_slice(&tag_count_val.to_le_bytes());

            if tag_data.len() <= 4 {
                ifd_data.extend_from_slice(&tag_data);
                ifd_data.extend_from_slice(&[0u8; 4][..4 - tag_data.len()]);
            } else {
                let offset: u32 = 8 + ifd_data.len() as u32 + 4;
                ifd_data.extend_from_slice(&offset.to_le_bytes());
            }
            tag_count += 1;
        }

        // Rebuild output - simple approach: insert new APP1 after SOI
        let mut result = Vec::new();
        ...
        // Copy everything up to and including SOI
        result.extend_from_slice(&buffer[..soi_pos.min(buffer.len())]);

        // Insert new APP1
        let total_exif_len: u16 = 2 + 4 + 2 + (tag_count as u16 * 12) + 4 + ifd_data.len() as u16;
        let app1_marker: [u8; 2] = [0xFF, 0xE1];
        result.extend_from_slice(&app1_marker);
        result.extend_from_slice(&total_exif_len.to_be_bytes());
        result.extend_from_slice(b"Exif");
        result.extend_from_slice(&[0u8; 2]); // Padding
        result.extend_from_slice(&42u16.to_le_bytes());
        result.extend_from_slice(&((8 + 2 + (tag_count as usize * 12)) as u32).to_le_bytes());
        result.extend_from_slice(&tag_count.to_le_bytes());
        result.extend_from_slice(&ifd_data);
        // Exif IFD pointer (0)
        result.extend_from_slice(&0u32.to_le_bytes());

        // Append rest of original file after SOI
        if soi_pos < buffer.len() {
            result.extend_from_slice(&buffer[soi_pos..]);
        }

        std::fs::write(path, result).map_err(|e| format!("Cannot write file: {e}"))?;
        Ok(())
    }

    fn write_png_exif(_path: &str, fields: Vec<[String; 2]>) -> Result<(), String> {
        // PNG EXIF support is limited - just clear GPS for now
        let mut has_gps_clear = false;
        for [tag, _] in &fields {
            if tag == "GPSLatitude" || tag == "GPSLongitude" || tag == "GPSAltitude" {
                has_gps_clear = true;
            }
        }

        if !has_gps_clear {
            return Err("PNG EXIF writing limited to GPS clear".to_string());
        }

        // For now, just don't write anything - PNG EXIF is complex
        // In a full implementation, would parse and rewrite tEXt/zTXt chunks
        Ok(())
    }

    // ═══════════════════════════════════════════════════════════════════════════════
    // Phase 6 — Native PDF metadata (no exiftool dependency)
    // ═══════════════════════════════════════════════════════════════════════════════
    ...
    #[tauri::command]
    fn get_pdf_meta(path: String) -> Result<PdfMeta, String> {
        let doc = lopdf::Document::load(&path).map_err(|e| format!("Cannot load PDF: {e}"))?;

        let mut meta = PdfMeta::default();

        if let Some(info) = doc.trailer.get(b"Info").ok().and_then(|r| r.as_reference().ok()) {
            if let Ok(dict) = doc.get_dictionary(info) {
                meta.title = get_pdf_string(&doc, &dict, b"Title");
                meta.author = get_pdf_string(&doc, &dict, b"Author");
                meta.subject = get_pdf_string(&doc, &dict, b"Subject");
                meta.keywords = get_pdf_string(&doc, &dict, b"Keywords");
            }
        }
    ...
    fn get_pdf_string(_doc: &lopdf::Document, dict: &lopdf::Dictionary, key: &[u8]) -> Option<String> {
        dict.get(key)
            .ok()
            .and_then(|v| match v {
    ...
    #[tauri::command]
    fn write_pdf_meta(path: String, fields: Vec<[String; 2]>) -> Result<(), String> {
        if fields.is_empty() {
            return Ok(());
        }

        let mut doc = lopdf::Document::load(&path).map_err(|e| format!("Cannot load PDF: {e}"))?;
        ...
        let info_dict = match info_ref {
            Some(ref_id) => {
                doc.get_dictionary_mut(ref_id).map_err(|e| format!("Cannot get Info dict: {e}"))?
            }
            None => {
                let new_info = lopdf::Dictionary::new();
                let info_ref = doc.add_object(new_info);
                doc.trailer.set("Info", lopdf::Object::Reference(info_ref));
                doc.get_dictionary_mut(info_ref).map_err(|e| format!("Cannot create Info dict: {e}"))?
            }
        };
        ...
        for [key, value] in &fields {
            ...
            info_dict.set(
                key.as_bytes(),
                lopdf::Object::String(bytes.to_vec(), lopdf::StringFormat::Literal),
            );
        }

        doc.save(&path).map_err(|e| format!("Cannot save PDF: {e}"))?;
        Ok(())
    }
    ...
    #[tauri::command]
    async fn compare_dirs(path_left: String, path_right: String) -> Result<DirDiffResult, String> {
        use sha2::{Digest, Sha256};
        use std::collections::HashMap;
        ...
        tauri::async_runtime::spawn_blocking(move || {
            ...
            for (rel, (sz_l, mt_l, is_dir)) in &map_l {
                ...
                if let Some(&(sz_r, mt_r, _)) = map_r.get(rel) {
                    let status = if sz_l == &sz_r && mt_l == &mt_r {
                        ...
                        let pl = root_l.join(rel);
                        let pr = root_r.join(rel);
                    ...
                    entries.push(DirDiffEntry {
                        ...
                        is_dir: *is_dir,
                        size_left: Some(*sz_l),
                        size_right: Some(sz_r),
                        mtime_left: Some(*mt_l),
                        mtime_right: Some(mt_r),
                    });
                } else {
                    entries.push(DirDiffEntry {
                        rel_path: rel.clone(),
            ...
            let only_left = entries
                .iter()
                .filter(|e| e.status == DiffStatus::OnlyLeft)
                .count();
            let only_right = entries
                .iter()
                .filter(|e| e.status == DiffStatus::OnlyRight)
                .count();
            let same = entries
                .iter()
                .filter(|e| e.status == DiffStatus::Same)
                .count();
            let different = entries
                .iter()
                .filter(|e| e.status == DiffStatus::Different)
                .count();
    ...
    pub struct SyncPreview {
        pub to_copy: Vec<SyncItem>,
        pub to_update: Vec<SyncItem>,
        pub to_delete: Vec<SyncItem>,
        pub total_size_copy: u64,
        pub total_size_update: u64,
    }
    ...
    fn get_file_meta(path: &Path) -> Option<(u64, i64)> {
        let meta = std::fs::metadata(path).ok()?;
        let size = meta.len();
        let mtime = meta
            .modified()
    ...
    fn walk_dir_for_sync(
        root: &Path,
        base: &Path,
    ) -> std::collections::HashMap<String, (u64, i64, bool)> {
        let mut map = std::collections::HashMap::new();
        fn walk(
            dir: &Path,
            base: &Path,
            map: &mut std::collections::HashMap<String, (u64, i64, bool)>,
        ) {
            if let Ok(entries) = std::fs::read_dir(dir) {
                for entry in entries.flatten() {
                    ...
                    let rel = path
                        ...
                        .replace('\\', "/");
                    if path.is_dir() {
                        map.insert(rel.clone(), (0, 0, true));
                        walk(&path, base, map);
                    } else if let Some((size, mtime)) = get_file_meta(&path) {
                        map.insert(rel, (size, mtime, false));
                    }
                }
            }
        }
        walk(root, base, &mut map);
        map
    }

    #[tauri::command]
    async fn sync_folders_preview(left: String, right: String) -> Result<SyncPreview, String> {
        if !Path::new(&left).exists() {
            return Err(format!("Source folder does not exist: {}", left));
        }
        if !Path::new(&right).exists() {
            return Err(format!("Destination folder does not exist: {}", right));
        }

        tauri::async_runtime::spawn_blocking(move || {
            let left_path = PathBuf::from(&left);
            let right_path = PathBuf::from(&right);
            let left_files = walk_dir_for_sync(&left_path, &left_path);
            let right_files = walk_dir_for_sync(&right_path, &right_path);

            let mut to_copy = Vec::new();
            let mut to_update = Vec::new();
            let mut to_delete = Vec::new();
            let mut total_size_copy = 0u64;
            let mut total_size_update = 0u64;

            // Files only in left -> copy to right
            for (rel, (size, mtime, is_dir)) in &left_files {
                if let Some((_, mtime_r, is_dir_r)) = right_files.get(rel) {
                    if *is_dir != *is_dir_r {
                        // Type changed
                        if !is_dir {
                            to_copy.push(SyncItem {
                                path: rel.clone(),
                                size: *size,
                                mtime: *mtime,
                                is_dir: *is_dir,
                            });
                            total_size_copy += size;
                        }
                    } else if !is_dir && *mtime != *mtime_r {
                        // Same file, different mtime -> update
                        to_update.push(SyncItem {
                            path: rel.clone(),
                            size: *size,
                            mtime: *mtime,
                            is_dir: false,
                        });
                        total_size_update += size;
                    }
                } else {
                    // Only in left
                    if *is_dir {
            ...
            // Files only in right -> delete (bidirectional sync removes orphans)
            for (rel, (size, mtime, is_dir)) in &right_files {
                if !left_files.contains_key(rel) {
                    to_delete.push(SyncItem {
                        path: rel.clone(),
                        size: *size,
                        mtime: *mtime,
                        is_dir: *is_dir,
    ...
    async fn sync_folders(
        ...
        // First get preview
        let preview = sync_folders_preview(left.clone(), right.clone()).await?;
        ...
        let _ = window.emit(
            "sync-progress",
            serde_json::json!({
                "done": 0, "total": total, "phase": "copying", "finished": false
            }),
        );

        // Copy new files
        for item in &preview.to_copy {
            let src = left_path.join(&item.path);
            let dst = right_path.join(&item.path);

            if item.is_dir {
                if let Err(e) = fs::create_dir_all(&dst) {
                    errors.push(format!("Failed to create dir {}: {}", item.path, e));
                    continue;
                }
            } else {
                ...
                match fs::copy(&src, &dst) {
                    Ok(_) => copied += 1,
                    Err(e) => errors.push(format!("Failed to copy {}: {}", item.path, e)),
                }
            }

            let _ = window.emit("sync-progress", serde_json::json!({
            "done": copied + updated + deleted, "total": total, "phase": "copying", "finished": false
        }));
        }
        ...
        for item in &preview.to_update {
            ...
            match fs::copy(&src, &dst) {
                Ok(_) => updated += 1,
                Err(e) => errors.push(format!("Failed to update {}: {}", item.path, e)),
            }

            let _ = window.emit("sync-progress", serde_json::json!({
            "done": copied + updated + deleted, "total": total, "phase": "updating", "finished": false
        }));
        }
            ...
            for item in &preview.to_delete {
                ...
                if item.is_dir {
                    match fs::remove_dir_all(&dst) {
                        Ok(_) => deleted += 1,
                        Err(e) => errors.push(format!("Failed to delete dir {}: {}", item.path, e)),
                    }
                } else {
                    match fs::remove_file(&dst) {
                        Ok(_) => deleted += 1,
                        Err(e) => errors.push(format!("Failed to delete {}: {}", item.path, e)),
                    }
                }

                let _ = window.emit("sync-progress", serde_json::json!({
                "done": copied + updated + deleted, "total": total, "phase": "deleting", "finished": false
            }));
            }
        }
        ...
        let _ = window.emit(
            "sync-progress",
            serde_json::json!({
                "done": total, "total": total, "phase": "done", "finished": true
            }),
        );
    ...
    fn get_versions_dir() -> PathBuf {
        dirs::data_local_dir()
            ...
            .join("frostfinder/versions")
    }

    fn version_file_path(original_path: &str, version_id: &str) -> PathBuf {
        // Create a safe filename from the original path
        let safe_name = original_path.replace(['/', '\\', ':'], "_");
        get_versions_dir().join(format!("{}_{}", safe_name, version_id))
    }

    fn version_meta_path(original_path: &str) -> PathBuf {
        let safe_name = original_path.replace(['/', '\\', ':'], "_");
        get_versions_dir().join(format!("{}.meta.json", safe_name))
    }
    ...
    fn load_version_meta(original_path: &str) -> VersionMeta {
        let meta_path = version_meta_path(original_path);
    ...
    fn save_version_meta(meta: &VersionMeta) -> Result<(), String> {
        let meta_path = version_meta_path(&meta.original_path);
    ...
    #[tauri::command]
    async fn create_version(path: String, note: Option<String>) -> Result<FileVersion, String> {
        let original = Path::new(&path);
        if !original.exists() {
            return Err(format!("File does not exist: {}", path));
        }
        if !original.is_file() {
            return Err("Can only version regular files".to_string());
        }

        let meta = std::fs::metadata(&original).map_err(|e| e.to_string())?;
        let size = meta.len();
        ...
        let timestamp = std::time::SystemTime::now()
            ...
            .map_err(|e| e.to_string())?
            .as_secs() as i64;
        let version_id = format!("v{}", timestamp);
        ...
        let version = FileVersion {
            ...
            version_path: String::new(), // filled after copy
        ...
        // Copy file to version store
        let version_path = version_file_path(&path, &version_id);
        if let Some(parent) = version_path.parent() {
            fs::create_dir_all(parent).map_err(|e| e.to_string())?;
        }
        fs::copy(&original, &version_path).map_err(|e| e.to_string())?;

        let mut version = version;
        version.version_path = version_path.to_string_lossy().to_string();

        // Update metadata
        let mut meta = load_version_meta(&path);
        meta.versions.push(version.clone());
        save_version_meta(&meta)?;

        Ok(version)
    }

    #[tauri::command]
    async fn list_versions(path: String) -> Result<Vec<FileVersion>, String> {
        let meta = load_version_meta(&path);
        Ok(meta.versions)
    }

    #[tauri::command]
    async fn restore_version(path: String, version_id: String) -> Result<String, String> {
        let meta = load_version_meta(&path);
        let version = meta
            .versions
            .iter()
        ...
        let version_path = Path::new(&version.version_path);
        if !version_path.exists() {
            return Err("Version file missing from storage".to_string());
        }
        ...
        if Path::new(&path).exists() {
            create_version(
                path.clone(),
                Some("Auto-backup before version restore".to_string()),
            )
            .await?;
        }

        fs::copy(version_path, &path).map_err(|e| e.to_string())?;
        Ok(format!("Restored {} to version {}", path, version_id))
    }

    #[tauri::command]
    async fn delete_version(path: String, version_id: String) -> Result<(), String> {
        let mut meta = load_version_meta(&path);
        let version_pos = meta
            .versions
            .iter()
            .position(|v| v.id == version_id)
            .ok_or_else(|| format!("Version not found: {}", version_id))?;

        let version = meta.versions.remove(version_pos);

        // Delete the version file
        let version_path = Path::new(&version.version_path);
        if version_path.exists() {
            fs::remove_file(version_path).map_err(|e| e.to_string())?;
        }
    ...
    #[tauri::command]
    async fn prune_versions(path: String, keep_last: Option<usize>) -> Result<usize, String> {
        let keep = keep_last.unwrap_or(5);
        if keep == 0 {
            return Err("Must keep at least 1 version".to_string());
        }

        let mut meta = load_version_meta(&path);
        if meta.versions.len() <= keep {
            return Ok(0);
        }
        ...
        let to_delete: Vec<_> = meta.versions.drain(keep..).collect();
    ...
    #[tauri::command]
    async fn get_version_stats() -> Result<serde_json::Value, String> {
        ...
        let mut files_with_versions = 0usize;

        fn count_versions(
            dir: &Path,
            total_versions: &mut usize,
            total_size: &mut u64,
            files_with_versions: &mut usize,
        ) {
            if let Ok(entries) = fs::read_dir(dir) {
                ...
                for entry in entries.flatten() {
                    let path = entry.path();
                    if path.is_dir() {
                        count_versions(&path, total_versions, total_size, files_with_versions);
                ...
                for meta_file in meta_files {
                    if let Ok(data) = fs::read_to_string(&meta_file) {
                        if let Ok(meta) = serde_json::from_str::<VersionMeta>(&data) {
                            *total_versions += meta.versions.len();
    ...
    fn get_signing_key_path() -> PathBuf {
        dirs::data_local_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("frostfinder/signing_key.bin")
    }
    ...
    #[tauri::command]
    fn generate_signing_key() -> Result<SigningKeyPair, String> {
        ...
        fs::write(&key_path, signing_key.to_bytes()).map_err(|e| e.to_string())?;
    ...
    #[tauri::command]
    fn get_signing_key() -> Result<Option<SigningKeyPair>, String> {
        let key_path = get_signing_key_path();
        if !key_path.exists() {
            return Ok(None);
        }
        ...
        let key_array: [u8; 32] = key_bytes.try_into().map_err(|_| "Invalid key")?;
    ...
    #[tauri::command]
    fn sign_file(path: String) -> Result<String, String> {
        use ed25519_dalek::{Signer, SigningKey};

        let key_path = get_signing_key_path();
        if !key_path.exists() {
            return Err(
                "No signing key found. Generate one first with generate_signing_key()".to_string(),
            );
        }
    ...
    #[tauri::command]
    fn delete_signing_key() -> Result<(), String> {
        let key_path = get_signing_key_path();
        if key_path.exists() {
    ...
    async fn split_file(
        ...
        if chunk_size < 1024 {
            return Err("Chunk size must be at least 1KB".to_string());
        }
        ...
        if file_size == 0 {
            return Err("Cannot split empty file".to_string());
        }
        ...
        for i in 0..total_chunks {
            let chunk_num = i + 1;
            let output_name = format!("{}.{:03}", file_name, chunk_num);
            let output_path = base_dir.join(&output_name);

            let bytes_read =
                std::io::Read::read(&mut input, &mut buffer).map_err(|e| e.to_string())?;
            if bytes_read > 0 {
            ...
            let _ = window.emit(
                "split-progress",
                serde_json::json!({
                    "done": chunk_num, "total": total_chunks, "finished": false
                }),
            );
        }
        ...
        let _ = window.emit(
            "split-progress",
            serde_json::json!({
                "done": total_chunks, "total": total_chunks, "finished": true
            }),
        );
    ...
    async fn join_files(
        ...
        for (i, path) in paths.iter().enumerate() {
            ...
            let _ = window.emit(
                "join-progress",
                serde_json::json!({
                    "done": i + 1, "total": total, "finished": false
                }),
            );
        }
    ...
    #[tauri::command]
    async fn encrypt_file(path: String, password: String) -> Result<String, String> {
        use aes::Aes256;
        use cbc::{
        ...
        let iv: [u8; 16] = Sha256::digest(format!("{:?}", std::time::SystemTime::now()).as_bytes())
            [..16]
            .try_into()
            .map_err(|_| "IV error")?;
        ...
        cipher.encrypt_block_mut(aes::cipher::generic_array::GenericArray::from_mut_slice(
            &mut padded[..16],
        ));
        for chunk in padded[16..].chunks_mut(16) {
            cipher.encrypt_block_mut(aes::cipher::generic_array::GenericArray::from_mut_slice(
                chunk,
            ));
        }
        ...
        let mut output = fs::File::create(&enc_path).map_err(|e| e.to_string())?;
        output.write_all(&iv).map_err(|e| e.to_string())?;
        output.write_all(&padded).map_err(|e| e.to_string())?;

        Ok(enc_path)
    }

    #[tauri::command]
    async fn decrypt_file(path: String, password: String) -> Result<String, String> {
        use aes::Aes256;
        use cbc::{
        ...
        let mut data = fs::read(&input_path).map_err(|e| e.to_string())?;
        if data.len() < 16 {
            return Err("Invalid encrypted file".to_string());
        }

        // Extract IV and encrypted data
        let iv: [u8; 16] = data[..16].try_into().map_err(|_| "IV error")?;
        ...
        cipher.decrypt_block_mut(aes::cipher::generic_array::GenericArray::from_mut_slice(
            &mut data[16..32],
        ));
        for chunk in data[32..].chunks_mut(16) {
            cipher.decrypt_block_mut(aes::cipher::generic_array::GenericArray::from_mut_slice(
                chunk,
            ));
        }
        ...
        let dec_path = path.trim_end_matches(".enc");
        let output_path = if dec_path == path {
            format!("{}.dec", path)
        } else {
            dec_path.to_string()
        };
        fs::write(&output_path, &data[16..]).map_err(|e| e.to_string())?;
    ...
    fn get_comments_path() -> PathBuf {
        dirs::data_local_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("frostfinder/comments.json")
    }
    ...
    fn load_comments() -> CommentsDb {
        let path = get_comments_path();
    ...
    fn save_comments(db: &CommentsDb) -> Result<(), String> {
        let path = get_comments_path();
    ...
    #[tauri::command]
    fn set_file_comment(path: String, comment: String) -> Result<(), String> {
        let mut db = load_comments();
        if comment.is_empty() {
            db.comments.remove(&path);
        } else {
            db.comments.insert(path, comment);
        }
        save_comments(&db)
    }

    #[tauri::command]
    fn get_file_comment(path: String) -> Option<String> {
        let db = load_comments();
        db.comments.get(&path).cloned()
    }

    #[tauri::command]
    fn get_all_comments() -> std::collections::HashMap<String, String> {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            get_tag_palette,
            set_tag_color,
            get_tags_with_colors,
            deep_search,
            get_tags_with_colors_v2,
            set_tag_color_v2,
            get_all_tags_v2,
            migrate_tags_to_sqlite,
            compress_files,
            extract_archive,
            ...
            trash_list,
            trash_item_count,
            check_trash_restore_conflicts,
            trash_restore_with_resolution,
            mount_sftp,
            unmount_sftp,
            get_sftp_mounts,
            mount_ftp,
        ...
        .setup(|app| {
            ...
            #[cfg(target_os = "linux")]
            {
                std::thread::spawn(|| {
                    ...
                    let icons: &[IconSize] = &[
                        IconSize {
                            data: include_bytes!("../icons/16x16.png"),
                            dir: "16x16",
                        },
                        IconSize {
                            data: include_bytes!("../icons/32x32.png"),
                            dir: "32x32",
                        },
                        IconSize {
                            data: include_bytes!("../icons/48x48.png"),
                            dir: "48x48",
                        },
                        IconSize {
                            data: include_bytes!("../icons/64x64.png"),
                            dir: "64x64",
                        },
                        IconSize {
                            data: include_bytes!("../icons/128x128.png"),
                            dir: "128x128",
                        },
                        IconSize {
                            data: include_bytes!("../icons/256x256.png"),
                            dir: "256x256",
                        },
                        IconSize {
                            data: include_bytes!("../icons/512x512.png"),
                            dir: "512x512",
                        },
                    ];
                    if let Some(home) = std::env::var_os("HOME") {
                        let base = std::path::Path::new(&home).join(".local/share/icons/hicolor");
                        ...
                        if any_written {
                            let _ = std::process::Command::new("gtk-update-icon-cache")
                                .args(["-f", "-t", base.to_string_lossy().as_ref()])
                                .status();
                        }
                    }
                });
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                if !saved.is_empty() {
                    ...
                    let still_mounted: Vec<SmbShare> = saved
                        .into_iter()
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                if !saved.is_empty() {
                    let proc_mounts = std::fs::read_to_string("/proc/mounts").unwrap_or_default();
                    let still_mounted: Vec<CloudMount> = saved
                        .into_iter()
...
fn sftp_reg_path() -> PathBuf {
    dirs::data_local_dir()
        .unwrap_or_else(|| PathBuf::from("/tmp"))
        .join("frostfinder/sftp_mounts.json")
}
fn sftp_reg_save(m: &[SftpMount]) {
    if let Some(p) = sftp_reg_path().parent() {
        let _ = std::fs::create_dir_all(p);
    }
    if let Ok(j) = serde_json::to_string(m) {
        let _ = std::fs::write(sftp_reg_path(), j);
    }
}
pub fn sftp_reg_load() -> Vec<SftpMount> {
    ...
    let mounts: Vec<SftpMount> = match serde_json::from_str(&json) {
        Ok(m) => m,
        Err(_) => return Vec::new(),
    };
    ...
    mounts
        .into_iter()
        .filter(|m| proc.contains(&m.mount_path))
        .collect()
}
...
fn mount_sftp(
    ...
    let status = if !password.is_empty() {
        ...
        if !key_path.is_empty() {
            c.arg("-o").arg(format!("IdentityFile={key_path}"));
        }
    ...
    if !status.success() {
        ...
        return Err("SFTP mount failed".into());
    }
    let mount = SftpMount {
        id: id.clone(),
        label: format!("{username}@{host}:{remote_path}"),
        host,
        port,
        username,
        remote_path,
...
fn unmount_sftp(id: String) -> Result<(), String> {
    ...
    if let Some(pos) = mounts.iter().position(|m| m.id == id) {
        ...
        let _ = std::process::Command::new("fusermount")
            ...
            .arg(&mp)
            .status()
            .or_else(|_| std::process::Command::new("umount").arg(&mp).status());
...
#[tauri::command]
fn get_sftp_mounts() -> Vec<SftpMount> {
    SFTP_MOUNTS
        .lock()
        .unwrap_or_else(|e| e.into_inner())
        .clone()
...
fn ftp_reg_path() -> PathBuf {
    dirs::data_local_dir()
        .unwrap_or_else(|| PathBuf::from("/tmp"))
        .join("frostfinder/ftp_mounts.json")
}
fn ftp_reg_save(m: &[FtpMount]) {
    if let Some(p) = ftp_reg_path().parent() {
        let _ = std::fs::create_dir_all(p);
    }
    if let Ok(j) = serde_json::to_string(m) {
        let _ = std::fs::write(ftp_reg_path(), j);
    }
}
pub fn ftp_reg_load() -> Vec<FtpMount> {
    ...
    let mounts: Vec<FtpMount> = match serde_json::from_str(&json) {
        Ok(m) => m,
        Err(_) => return Vec::new(),
    };
    let proc = std::fs::read_to_string("/proc/mounts").unwrap_or_default();
...
fn mount_ftp(
    ...
    cmd.arg(&url)
        .arg(&mount_path)
        .arg("-o")
        .arg(format!("user={username}:{password}"))
        .arg("-o")
        .arg("allow_other");
    if passive {
        cmd.arg("-o").arg("ftp_port=-");
    }
    if tls {
        cmd.arg("-o").arg("ssl");
    }
    let status = cmd
        .status()
        .map_err(|e| format!("curlftpfs not found: {e}"))?;
    if !status.success() {
        let _ = std::fs::remove_dir(&mount_path);
        return Err("FTP mount failed".into());
    }
    let mount = FtpMount {
        id: id.clone(),
        label: format!("{username}@{host}{remote_path}"),
...
#[tauri::command]
fn get_ftp_mounts() -> Vec<FtpMount> {
    FTP_MOUNTS.lock().unwrap_or_else(|e| e.into_inner()).clone()
}
...
fn get_file_permissions(path: String) -> Result<FilePermissionsInfo, String> {
    ...
    let owner = {
        let pw = unsafe { libc::getpwuid(meta.uid()) };
        if pw.is_null() {
            meta.uid().to_string()
        } else {
    ...
    let group = {
        let gr = unsafe { libc::getgrgid(meta.gid()) };
        if gr.is_null() {
            meta.gid().to_string()
        } else {
    ...
    let created = meta
        ...
        .and_then(|t| t.duration_since(UNIX_EPOCH).ok())
        .map(|d| d.as_secs());
    let mime_hint = p.extension().and_then(|e| e.to_str()).map(|e| {
...
fn chown_entry(path: String, owner: String, group: String) -> Result<(), String> {
    let resolve_uid = |name: &str| -> Option<u32> {
    ...
    let resolve_gid = |name: &str| -> Option<u32> {
        if let Ok(n) = name.parse::<u32>() {
            return Some(n);
        }
        let c = std::ffi::CString::new(name).ok()?;
    ...
    let uid = if owner.is_empty() {
        ...
        resolve_uid(&owner).ok_or_else(|| format!("Unknown user: {owner}"))?
    };
    let gid = if group.is_empty() {
        !0u32
    } else {
        resolve_gid(&group).ok_or_else(|| format!("Unknown group: {group}"))?
    };
...
fn scan_dir_sizes(path: String) -> Result<Vec<DirSizeEntry>, String> {
    fn rec(p: &Path, visited: &mut std::collections::HashSet<u64>) -> u64 {
        ...
        std::fs::read_dir(p)
            .map(|rd| rd.flatten().map(|e| rec(&e.path(), visited)).sum())
            .unwrap_or(0)
    }
    let mut entries: Vec<DirSizeEntry> = std::fs::read_dir(Path::new(&path))
        .map_err(|e| e.to_string())?
        .flatten()
        .take(256)
        .map(|e| {
...
#[cfg(target_os = "windows")]
fn xattr_write_tags(_path: &str, _tags: &[String]) -> bool {
    false
}
...
fn probe_video_codec(path: String) -> Result<Option<VideoCodecInfo>, String> {
    ...
    let fps = video.and_then(|v| v["r_frame_rate"].as_str()).map(|s| {
        if let Some((n, d)) = s.split_once('/') {
            ...
            let df: f64 = d.parse().unwrap_or(1.0);
            format!("{:.3}", if df != 0.0 { nf / df } else { 0.0 })
        } else {
            s.to_string()
        }
    });
    Ok(Some(VideoCodecInfo {
        ...
        fps,
        duration_secs: fmt["duration"].as_str().and_then(|s| s.parse::<f64>().ok()),
        bit_rate_kbps: fmt["bit_rate"]
            .as_str()
...
fn save_plugins(plugins: Vec<PluginDef>) -> Result<(), String> {
    if let Some(p) = plugins_path().parent() {
        let _ = std::fs::create_dir_all(p);
    }
...
fn plugin_trust_path() -> PathBuf {
    dirs::data_local_dir()
        .unwrap_or_else(|| PathBuf::from("/tmp"))
        .join("frostfinder/plugin_trust.json")
}
...
fn plugin_trust_save(map: &std::collections::HashMap<String, u64>) {
    if let Some(parent) = plugin_trust_path().parent() {
        let _ = std::fs::create_dir_all(parent);
    }
    if let Ok(j) = serde_json::to_string(map) {
        let _ = std::fs::write(plugin_trust_path(), j);
    }
}
fn djb2(s: &str) -> u64 {
    s.bytes()
...
fn check_plugin_trust(plugin_id: String, command: String) -> serde_json::Value {
    ...
    match store.get(&plugin_id) {
        ...
        Some(&stored_hash) if stored_hash != hash => {
            serde_json::json!({"trusted": false, "changed": true, "first_run": false})
        }
...
/// r27 P1.2: Remove a plugin's trust entry so it re-prompts on next run.
/// Called when user clicks "Revoke" in Plugin Manager or when a plugin is deleted.
#[tauri::command]
fn revoke_plugin_trust(plugin_id: String) {
    let mut store = plugin_trust_load();
...
fn run_plugin_command(
    ...
    if let Some(se) = child.stderr.as_mut() {
        ...
        if stderr_str.len() > OUTPUT_CAP {
            stderr_str.truncate(OUTPUT_CAP);
            stderr_str.push_str("\n[stderr truncated]");
        }
    }
    Ok(PluginResult {
...
// ─────────────────────────────────────────────────────────────────────────────
// r30 P3.1: Persistent undo history — serialise/restore undo stack across sessions
// ─────────────────────────────────────────────────────────────────────────────

fn undo_history_path() -> std::path::PathBuf {
    dirs::data_local_dir()
        .unwrap_or_else(|| std::path::PathBuf::from("/tmp"))
        .join("frostfinder/undo-history.json")
}

/// Save the undo stack (JSON array, max 200 entries) to disk.
#[tauri::command]
fn save_undo_history(history: serde_json::Value) -> Result<(), String> {
    let path = undo_history_path();
    if let Some(p) = path.parent() {
        let _ = std::fs::create_dir_all(p);
    }
    // Enforce 200-entry cap server-side as a safety net
    let capped = if let Some(arr) = history.as_array() {
        let start = arr.len().saturating_sub(200);
        serde_json::Value::Array(arr[start..].to_vec())
    } else {
        serde_json::json!([])
    };
    std::fs::write(&path, capped.to_string()).map_err(|e| e.to_string())
}

/// Load the saved undo stack from disk. Returns empty array if no file exists.
#[tauri::command]
fn load_undo_history() -> serde_json::Value {
    let path = undo_history_path();
...
/// Clear the saved undo history file.
#[tauri::command]
fn clear_undo_history() -> Result<(), String> {
    let path = undo_history_path();
    if path.exists() {
        std::fs::remove_file(&path).map_err(|e| e.to_string())?;
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// r31 P4.3: Cloud sync-state awareness — per-file sync badges via rclone
// ─────────────────────────────────────────────────────────────────────────────
...
/// r31 P4.3: Query sync state for files in a directory via `rclone lsf`.
/// Returns local-only, synced, or cloud-only per file.
/// `cloud_remote` is the rclone remote name (e.g. "gdrive:", "s3:bucket/path").
#[tauri::command]
fn get_cloud_sync_state(local_dir: String, cloud_remote: String) -> Vec<FileSyncState> {
    if !which("rclone") {
        return vec![];
    }

    // List local files
    let local_files: std::collections::HashSet<String> = std::fs::read_dir(&local_dir)
        .ok()
        .map(|rd| {
            rd.flatten()
    ...
    // List cloud files via rclone lsf (--format isTe = isDir, size, mtime, name)
    let cloud_out = std::process::Command::new("rclone")
        .args(["lsf", "--format", "p", "--max-depth", "1", &cloud_remote])
        .output();
    let cloud_files: std::collections::HashSet<String> = cloud_out
        .ok()
        .map(|o| {
    ...
    for f in &local_files {
        if !cloud_files.contains(f) {
            result.push(FileSyncState {
                path: format!("{}/{}", local_dir.trim_end_matches('/'), f),
...
/// r31 P4.3: Check if an rclone remote is reachable (for offline indicator).
#[tauri::command]
fn check_cloud_remote_reachable(cloud_remote: String) -> bool {
    if !which("rclone") {
        return false;
    }
    std::process::Command::new("rclone")
        .args(["lsf", "--max-depth", "0", "--timeout", "5s", &cloud_remote])
        .output()
        .map(|o| o.status.success())
        .unwrap_or(false)
}
...
// ─────────────────────────────────────────────────────────────────────────────
// r32 P5.3: Native rclone cloud integration without FUSE
// Uses `rclone serve rc` — the rclone Remote Control HTTP API.
// Allows cloud browse/copy/move on Windows and macOS without FUSE/Dokany.
// ─────────────────────────────────────────────────────────────────────────────

static RCLONE_RC_PROCESS: OnceLock<Mutex<Option<std::process::Child>>> = OnceLock::new();

fn rclone_rc_mutex() -> &'static Mutex<Option<std::process::Child>> {
    RCLONE_RC_PROCESS.get_or_init(|| Mutex::new(None))
}

fn rclone_rc_port() -> u16 {
    5572
} // rclone default RC port

/// r32 P5.3: Start `rclone serve rc` daemon for no-FUSE cloud access.
/// Returns the base URL of the RC API.
#[tauri::command]
fn start_rclone_rc() -> Result<String, String> {
    if !which("rclone") {
        return Err("rclone not found".into());
    }
    let mut guard = rclone_rc_mutex().lock().unwrap_or_else(|e| e.into_inner());
    // Check if already running
    if let Some(child) = guard.as_mut() {
        match child.try_wait() {
            Ok(None) => return Ok(format!("http://localhost:{}", rclone_rc_port())),
            _ => {}
        }
    }
    let child = std::process::Command::new("rclone")
        .args([
            "rcd",
            "--rc-no-auth",
            &format!("--rc-addr=127.0.0.1:{}", rclone_rc_port()),
            "--rc-web-gui=false",
        ])
        .spawn()
        .map_err(|e| format!("rclone rcd failed to start: {e}"))?;
    *guard = Some(child);
    // Brief wait for the daemon to bind
    std::thread::sleep(std::time::Duration::from_millis(400));
    Ok(format!("http://localhost:{}", rclone_rc_port()))
}

/// r32 P5.3: Stop the rclone RC daemon.
#[tauri::command]
fn stop_rclone_rc() -> Result<(), String> {
    let mut guard = rclone_rc_mutex().lock().unwrap_or_else(|e| e.into_inner());
...
/// r32 P5.3: List files in a remote path via rclone RC API (no FUSE required).
/// `remote` is e.g. "gdrive:" or "s3:mybucket/path"
#[tauri::command]
fn rclone_rc_list(remote: String, path: String) -> Result<Vec<serde_json::Value>, String> {
    let url = format!("http://localhost:{}/operations/list", rclone_rc_port());
    let body = serde_json::json!({ "fs": remote, "remote": path });
    ...
    let items = resp["list"].as_array().cloned().unwrap_or_default();
    Ok(items)
}

/// r32 P5.3: Copy a file to/from a remote via rclone RC (no FUSE required).
#[tauri::command]
fn rclone_rc_copy(
    src_fs: String,
    src_path: String,
    ...
    let url = format!("http://localhost:{}/operations/copyfile", rclone_rc_port());
    let body = serde_json::json!({
        "srcFs": src_fs, "srcRemote": src_path,
        "dstFs": dst_fs, "dstRemote": dst_path,
    });
    let out = std::process::Command::new("curl")
        .args([
            "-s",
...
// ─────────────────────────────────────────────────────────────────────────────
// r31 P4.2: MTP / Android & camera device support
// ─────────────────────────────────────────────────────────────────────────────

#[derive(serde::Serialize, serde::Deserialize, Debug, Clone)]
pub struct MtpDevice {
    pub id: String,   // e.g. "mtp://[usb:001,004]/"
    pub name: String, // Friendly name from gvfs/gio
    pub vendor: String,
    pub model: String,
    pub mount_path: Option<String>, // None if not yet mounted
}

/// r31 P4.2: Detect MTP/PTP devices using `gio mount --list` or `lsusb` heuristics.
#[tauri::command]
fn detect_mtp_devices() -> Vec<MtpDevice> {
    // Primary: use gio to list gvfs-backed MTP mounts
    let gio_out = std::process::Command::new("gio")
        .args(["mount", "--list"])
        .output();
    let mut devices: Vec<MtpDevice> = Vec::new();

    if let Ok(out) = gio_out {
        let text = String::from_utf8_lossy(&out.stdout);
        for line in text.lines() {
            let line = line.trim();
            if line.starts_with("Mount(") && line.contains("mtp://") {
                // Parse: Mount(0): LG Phone -> mtp://[usb:001,004]/
                let name = line
                    .split("): ")
                    .nth(1)
                    .and_then(|s| s.split(" -> ").next())
                    ...
                    .trim()
                    .to_string();
                let uri = line.split(" -> ").last().unwrap_or("").trim().to_string();
                // Check if already mounted under gvfs
                let gvfs_path = format!(
                    "/run/user/{}/gvfs/{}",
                    unsafe { libc::getuid() },
                    uri.replace("://", ":")
                        .replace("/", "")
                ...
                let mount_path = if std::path::Path::new(&gvfs_path).exists() {
                    Some(gvfs_path)
                } else {
                    None
                };
    ...
    // Fallback: detect via lsusb if gio found nothing
    if devices.is_empty() && which("lsusb") {
        if let Ok(out) = std::process::Command::new("lsusb").output() {
            for line in String::from_utf8_lossy(&out.stdout).lines() {
                // Very rough heuristic — flag devices with known Android/camera vendor IDs
...
/// r31 P4.2: Mount an MTP device via `gio mount <uri>`.
#[tauri::command]
fn mount_mtp(device_id: String) -> Result<String, String> {
    if !which("gio") {
        return Err("gio not found — install gvfs".into());
    }
    let out = std::process::Command::new("gio")
        ...
        .map_err(|e| e.to_string())?;
    if out.status.success() {
        // Derive the gvfs mount path
        let uid = unsafe { libc::getuid() };
        let base = device_id
            .replace("://", ":")
            .replace(['/', '[', ']', ','], "_");
        let path = format!("/run/user/{}/gvfs/{}", uid, base);
        Ok(path)
    } else {
        Err(String::from_utf8_lossy(&out.stderr).trim().to_string())
    }
...
/// r31 P4.2: Unmount an MTP device via `gio mount --unmount <uri>`.
#[tauri::command]
fn unmount_mtp(device_id: String) -> Result<(), String> {
    if !which("gio") {
        return Err("gio not found".into());
    }
    let out = std::process::Command::new("gio")
        .args(["mount", "--unmount", &device_id])
        .output()
        .map_err(|e| e.to_string())?;
    if out.status.success() {
        Ok(())
...
mod tests {
    ...
    fn dir_cache_evicts_lru_at_capacity() {
        ...
        assert!(
            cache.get("/p0").is_some(),
            "p0 should still be present before overflow"
        );
        ...
        assert!(
            cache.get("/overflow").is_some(),
            "/overflow must be present"
        );
    }
    ...
    fn dir_cache_refresh_moves_to_back() {
        ...
        assert!(
            cache.get("/p0").is_some(),
            "/p0 was re-inserted, must survive"
        );
    ...
    fn smb_registry_empty_on_corrupt_json() {
        ...
        let loaded: Vec<SmbShare> =
            serde_json::from_str(&fs::read_to_string(&p).unwrap()).unwrap_or_default();
    ...
    fn sftp_mount_roundtrip() {
        let m = SftpMount {
            id: "sftp-1".into(),
            label: "Work Server".into(),
            host: "ssh.example.com".into(),
            port: 22,
            username: "dev".into(),
            remote_path: "/home/dev".into(),
    ...
    fn audit_tag_db_returns_orphans_only() {
        ...
        assert!(
            !orphans.contains(&real_path),
            "real file must not be orphan"
        );
    ...
    fn ftp_mount_tls_flag() {
        let plain = FtpMount {
            ...
            port: 21,
            username: "anon".into(),
            remote_path: "/pub".into(),
            mount_path: "/tmp/ff-ftp-1".into(),
            tls: false,
        };
        let tls = FtpMount {
            tls: true,
    ...
    fn batch_rename_prefix_prepends_to_stem() {
        ...
        let results =
            futures::executor::block_on(batch_rename(vec![p.to_string_lossy().to_string()], opts))
                .unwrap();
        assert_eq!(results.len(), 1);
    ...
    fn batch_rename_number_uses_start_num_and_padding() {
        let dir = tmp();
        let files: Vec<_> = (0..3)
            .map(|i| make_file(dir.path(), &format!("img{}.jpg", i), b""))
            .collect();
