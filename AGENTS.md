...
The plugin JSON schema is semver-stable as of v1.0. The schema is defined in
`plugin-schema-v1.json`. Breaking changes (removing fields, changing types) require
a major version bump. Additive changes (new optional fields) are allowed in minor versions.

**Stable IPC commands** (as of v1.0):
`load_plugins`, `save_plugins`, `run_plugin_command`, `check_plugin_trust`,
`approve_plugin`, `revoke_plugin_trust`. These command signatures will not change
in a breaking way within the v1.x release line.

### localStorage vs sessionStorage contract (r27 P1.3)

**Rule:** global preferences that should survive across all windows and sessions go in
`localStorage`. State that is specific to one window instance goes in `sessionStorage`.

| Key | Storage | Rationale |
|---|---|---|
| `ff_*` settings (viewMode, iconSize, locale, etc.) | `localStorage` | User preferences, global |
| `ff_session` (tab restore data) | `localStorage` | Main window session, intentionally shared so a crash can be recovered |
| `ff_pane_split` (divider fraction) | `localStorage` | Preference, not instance state |
| `ff_split_active` | `sessionStorage` | Per-window — each window manages its own split pane |
| `ff_win_num` | `sessionStorage` | Per-window numbering for title disambiguation |

**Contributor rule:** If your feature creates new persistent state, ask: "Should every open
FrostFinder window see the same value?" If yes → `localStorage`. If it describes the
state of *this particular window* → `sessionStorage`.
